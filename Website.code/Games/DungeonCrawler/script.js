...
const UI = {
    ...
    showSpellChoiceUI: function(spellChoices) {
        ...
        console.log('actionsDiv found, creating buttons for', spellChoices.length, 'spells');
        ...
        const disableAndRemoveAllChoices = () => {
            ...
            const controls = document.getElementById('controls');
            const removeBtn = (btn) => { btn.disabled = true; btn.remove(); };
            if (controls) {
                // Remove common choice buttons within controls
                controls.querySelectorAll('.spell-choice-btn, .choice-btn, .risk-btn, #confirm-vampire-spells-btn').forEach(removeBtn);
                // Also remove any buttons inside potential wrappers
                controls.querySelectorAll('#spell-choice, .spell-choice, .spell-choice-container, .replace-choice').forEach(container => {
                    container.querySelectorAll('button').forEach(removeBtn);
                    container.remove();
                });
                // Final wipe of controls container to guarantee removal
                controls.querySelectorAll('button').forEach(btn => {
                    if (!btn.id || btn.classList.contains('spell-choice-btn') || btn.classList.contains('choice-btn') || btn.classList.contains('risk-btn')) {
                        // keep primary controls by ID; remove any ad-hoc buttons
                        if (!['explore-btn','rest-btn','potion-btn','mana-potion-btn','shop-btn','attack-btn','spell-btn','run-btn'].includes(btn.id)) {
                            removeBtn(btn);
                        }
                    }
                });
            ...
            document.querySelectorAll('.spell-choice-btn, .choice-btn, .risk-btn, #confirm-vampire-spells-btn').forEach(removeBtn);
            document.querySelectorAll('#spell-choice, .spell-choice, .spell-choice-container, .replace-choice').forEach(el => {
            ...
            setTimeout(() => {
                ...
                if (actionsDiv) {
                    ...
                    actionsDiv.querySelectorAll('.spell-choice-btn, .choice-btn, .risk-btn, #confirm-vampire-spells-btn').forEach(removeBtn);
                }
            }, 0);
            this._spellChoiceActive = false;
        };
        ...
        spellChoices.forEach((spellName, index) => {
            ...
            btn.className = 'spell-choice-btn bg-gradient-to-r from-purple-600 to-purple-500 text-white px-4 py-3 rounded-lg font-bold';
            ...
            console.log(`Appended spell choice button for: ${spellName}, total buttons: ${actionsDiv.children.length}, button visibility: ${btn.style.visibility}, opacity: ${btn.style.opacity}, display: ${btn.style.display}`);
        });
        
        // Add skip button
        const skipBtn = document.createElement('button');
        skipBtn.innerHTML = `<i class="fas fa-forward"></i> Skip`;
        skipBtn.className = 'spell-choice-btn bg-gradient-to-r from-gray-500 to-gray-400 text-white px-4 py-3 rounded-lg font-bold';
    ...
    showSpellReplaceUI: function(newSpell) {
        ...
        gameState.character.knownSpells.forEach(spellName => {
            ...
            btn.className = 'spell-choice-btn bg-gradient-to-r from-red-500 to-red-400 text-white px-4 py-3 rounded-lg font-bold';
            btn.style.display = 'block';
    ...
    hideAllActionButtons: function(force = false) {
        ...
        if (this._spellChoiceActive && !force) {
            console.log('hideAllActionButtons skipped because _spellChoiceActive is true');
            return;
        }
