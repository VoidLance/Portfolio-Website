...
import {
  CHARACTER_CLASSES,
  CLASS_DESCRIPTIONS,
} from "./classes/CharacterClasses.js";
...
const gameState = {
  ...
  gameWin: function () {
    ...
    UI.updateStory(
      `<i class="fas fa-trophy"></i> <strong>VICTORY!</strong> You have reached the deepest chamber of the dungeon and found the legendary treasure! The realm is saved!`,
    );
  ...
  gameOver: function (reason = "death") {
    ...
    if (reason === "time") {
      UI.updateStory(
        `<i class="fas fa-hourglass-end"></i> <strong>TIME'S UP!</strong> You spent too long in the dungeon and became lost forever in its dark passages...`,
      );
      UI.addToLog(
        "<i class='fas fa-skull'></i> GAME OVER - Time limit exceeded!",
        "death",
      );
    } else if (reason === "victory") {
      UI.updateStory(
        `<i class="fas fa-trophy"></i> <strong>ULTIMATE VICTORY!</strong> You have conquered the deepest depths and defeated the Dungeon Lord! You emerge as a legendary hero, your name forever etched in the annals of greatness!`,
      );
      UI.addToLog(
        `<i class='fas fa-trophy'></i> VICTORY! Final Gold: ${this.inventory.gold}!`,
        "victory",
      );
      ...
      UI.updateStory(
        `<i class="fas fa-skull-crossbones"></i> <strong>GAME OVER!</strong> Your adventure ends here...`,
      );
      UI.addToLog(
        "<i class='fas fa-skull'></i> GAME OVER - No lives remaining!",
        "death",
      );
    }
  ...
  addExp: function (amount) {
    ...
    if (this.exp >= expNeeded) {
      ...
      UI.addToLog(
        `⭐ LEVEL UP! Level ${this.level}! (+${amount} XP)`,
        "levelup",
      );
  ...
  levelUpCharacter: function () {
    ...
    this.character.agility = Math.min(
      ...
      this.character.getMaxAgilityCap
        ? this.character.getMaxAgilityCap()
    ...
    if (rates.physicalMasteryPerLevel) {
      const newPhysicalMastery = Math.min(
        this.character.combatSpecialization.physicalMastery +
          rates.physicalMasteryPerLevel,
      ...
      if (masteryGain > 0) {
        this.character.combatSpecialization.physicalMastery =
          newPhysicalMastery;
        UI.addToLog(
          `<i class="fas fa-sword"></i> Combat expertise sharpens! +${masteryGain.toFixed(2)} Physical Mastery`,
          "levelup",
        );
      }
    }
    ...
    if (className === "Warrior" && this.level % 3 === 0) {
      ...
      if (newPhysical > this.character.combatSpecialization.physicalMastery) {
        ...
        UI.addToLog(
          `<i class="fas fa-shield"></i> Battle experience! +0.125 Physical Mastery`,
          "levelup",
        );
      }
      ...
      if (
        newPhysical > this.character.combatSpecialization.physicalMastery ||
        ...
        UI.addToLog(
          `<i class="fas fa-cross"></i> Divine balance! +0.25 Physical & Magical Mastery`,
          "levelup",
        );
      }
      ...
      if (newMagical > this.character.combatSpecialization.magicalMastery) {
        this.character.combatSpecialization.magicalMastery = newMagical;
        UI.addToLog(
          `<i class="fas fa-wand-magic"></i> Arcane knowledge! +0.25 Magical Mastery`,
          "levelup",
        );
      }
    }
  ...
  offerSpellChoice: function (level) {
    console.log("offerSpellChoice called for level:", level);
    if (!this.character || this.character.isVampire) {
      console.log("Skipping spell choice - no character or is vampire");
      return;
    }
    ...
    for (const spellName in GameData.genericSpells) {
      ...
      if (
        (spell.unlockLevel === level ||
          (spell.unlockLevel >= level - 1 && spell.unlockLevel < level)) &&
    ...
    console.log("Available spells for level", level, ":", availableSpells);
    if (availableSpells.length === 0) {
      console.log("No available spells for level", level);
      return;
    }
  ...
  learnSpell: function (spellName) {
    // Prevent duplicates
    if (this.character.knownSpells.includes(spellName)) {
      UI.addToLog(`You already know ${spellName}.`, "error");
      UI.updateDisplay();
      return false;
    }
    ...
    if (!spell) {
      console.error(
        `Failed to find spell: ${spellName}, isVampire: ${this.character.isVampire}`,
      );
      UI.addToLog(
        `<i class="fas fa-magic"></i> Learned: ${spellName}!`,
        "levelup",
      );
    } else {
      UI.addToLog(
        `<i class="fas fa-magic"></i> Learned: ${spellName}! ${spell.description}`,
        "levelup",
      );
    }
    UI.updateDisplay();
  ...
  replaceSpell: function (oldSpell, newSpell) {
    ...
    if (index !== -1) {
      ...
      if (!spell) {
        console.error(
          `Failed to find spell: ${newSpell}, isVampire: ${this.character.isVampire}`,
        );
        UI.addToLog(
          `<i class="fas fa-exchange-alt"></i> Replaced ${oldSpell} with ${newSpell}!`,
          "levelup",
        );
      } else {
        UI.addToLog(
          `<i class="fas fa-exchange-alt"></i> Replaced ${oldSpell} with ${newSpell}! ${spell.description}`,
          "levelup",
        );
      }
      if (this.character.selectedSpell === oldSpell)
        this.character.selectedSpell = newSpell;
      UI.updateDisplay();
    }
  },
  ...
  playerDeath: function () {
    ...
    if (this.lives <= 0) {
      ...
      UI.addToLog(
        `<i class='fas fa-skull'></i> You died! Respawning with ${this.lives} lives remaining...`,
        "death",
      );
      UI.updateStory(
        '<i class="fas fa-skull"></i> Death! You have been defeated but your spirit endures. You respawn at the dungeon entrance.',
      );
      Combat.endBattle();
    }
  },
...
const GameData = {
  ...
  weapons: [
    ...
    new EquipmentItem("Rusty Dagger", 2, "common", "agility", {
      classPreference: "Rogue",
    }),
    new EquipmentItem("Sharpened Dagger", 3, "common", "agility", {
      classPreference: "Rogue",
    }),
    new EquipmentItem("Assassin's Blade", 5, "uncommon", "agility", {
      classPreference: "Rogue",
    }),
    new EquipmentItem("Venomfang Dagger", 7, "uncommon", "agility", {
      classPreference: "Rogue",
    }),
    new EquipmentItem("Shadowstrike Dagger", 9, "rare", "agility", {
      classPreference: "Rogue",
    }),
    new EquipmentItem("Nightblade", 11, "rare", "agility", {
      classPreference: "Rogue",
    }),
    new EquipmentItem("Fang of the Abyss", 14, "epic", "agility", {
      classPreference: "Rogue",
    }),
    // Swords (Warrior/Paladin preference)
    new EquipmentItem("Iron Sword", 4, "common", "strength", {
      classPreference: "Warrior",
    }),
    new EquipmentItem("Steel Blade", 6, "uncommon", "strength", {
      classPreference: "Warrior",
    }),
    new EquipmentItem("Knight's Longsword", 8, "uncommon", "strength", {
      classPreference: "Paladin",
    }),
    new EquipmentItem("Executioner's Axe", 10, "rare", "strength", {
      classPreference: "Warrior",
    }),
    new EquipmentItem("Holy Avenger", 11, "rare", "strength", {
      classPreference: "Paladin",
    }),
    new EquipmentItem("Dragon Slayer", 12, "rare", "strength", {
      classPreference: "Warrior",
    }),
    new EquipmentItem("Demon's Greatsword", 15, "epic", "strength", {
      classPreference: "Warrior",
    }),
    // Staves/Wands (Mage preference)
    new EquipmentItem("Apprentice Wand", 2, "common", "power", {
      classPreference: "Mage",
    }),
    new EquipmentItem("Crystal Staff", 4, "common", "power", {
      classPreference: "Mage",
    }),
    new EquipmentItem("Enchanter's Rod", 6, "uncommon", "power", {
      classPreference: "Mage",
    }),
    new EquipmentItem("Sorcerer's Staff", 8, "uncommon", "power", {
      classPreference: "Mage",
    }),
    new EquipmentItem("Staff of the Archmage", 10, "rare", "power", {
      classPreference: "Mage",
    }),
    new EquipmentItem("Celestial Scepter", 12, "rare", "power", {
      classPreference: "Mage",
    }),
    new EquipmentItem("Staff of Ultimate Power", 15, "epic", "power", {
      classPreference: "Mage",
    }),
  ],
  ...
  armor: [
    ...
    new EquipmentItem("Leather Vest", 2, "common", "agility", {
      armorType: "light",
      classPreference: "Rogue",
    }),
    new EquipmentItem("Studded Leather", 4, "uncommon", "agility", {
      armorType: "light",
      classPreference: "Rogue",
    }),
    new EquipmentItem("Elven Mail", 6, "rare", "agility", {
      armorType: "light",
      classPreference: "Rogue",
    }),
    new EquipmentItem("Shadowweave Armor", 8, "rare", "agility", {
      armorType: "light",
      classPreference: "Rogue",
    }),
    new EquipmentItem("Dragon Scale Armor", 10, "epic", "agility", {
      armorType: "light",
      classPreference: "Rogue",
    }),
    // Heavy armor (Warrior/Paladin preference)
    new EquipmentItem("Chainmail Shirt", 8, "common", "defense", {
      armorType: "heavy",
      classPreference: "Warrior",
    }),
    new EquipmentItem("Steel Plate Armor", 14, "uncommon", "defense", {
      armorType: "heavy",
      classPreference: "Warrior",
    }),
    new EquipmentItem("Consecrated Plate", 16, "uncommon", "defense", {
      armorType: "heavy",
      classPreference: "Paladin",
    }),
    new EquipmentItem("Knight's Full Plate", 20, "rare", "defense", {
      armorType: "heavy",
      classPreference: "Paladin",
    }),
    new EquipmentItem("Battlelord's Armor", 22, "rare", "defense", {
      armorType: "heavy",
      classPreference: "Warrior",
    }),
    new EquipmentItem("Mithril Armor", 28, "epic", "defense", {
      armorType: "heavy",
      classPreference: "Warrior",
    }),
    // Robes (Mage preference)
    new EquipmentItem("Apprentice Robes", 3, "common", "power", {
      armorType: "cloth",
      classPreference: "Mage",
    }),
    new EquipmentItem("Arcane Vestments", 5, "uncommon", "power", {
      armorType: "cloth",
      classPreference: "Mage",
    }),
    new EquipmentItem("Sorcerer's Robes", 7, "rare", "power", {
      armorType: "cloth",
      classPreference: "Mage",
    }),
    new EquipmentItem("Robes of the Archmage", 9, "epic", "power", {
      armorType: "cloth",
      classPreference: "Mage",
    }),
  ],
  accessories: [
    ...
    new EquipmentItem("Silver Ring", 1, "common", "agility", {
      classPreference: "Rogue",
    }),
    new EquipmentItem("Amulet of Swiftness", 2, "common", "agility", {
      classPreference: "Rogue",
    }),
    new EquipmentItem("Boots of Speed", 4, "uncommon", "agility", {
      classPreference: "Rogue",
    }),
    new EquipmentItem("Cloak of Shadows", 5, "rare", "agility", {
      classPreference: "Rogue",
    }),
    new EquipmentItem("Shadowdancer's Mantle", 7, "epic", "agility", {
      classPreference: "Rogue",
    }),
    // Defense accessories (Warrior/Paladin preference)
    new EquipmentItem("Iron Bracers", 2, "common", "defense", {
      classPreference: "Warrior",
    }),
    new EquipmentItem("Guardian Amulet", 3, "uncommon", "defense", {
      classPreference: "Paladin",
    }),
    new EquipmentItem("Shield Ring", 4, "rare", "defense", {
      classPreference: "Warrior",
    }),
    new EquipmentItem("Bulwark Medallion", 6, "epic", "defense", {
      classPreference: "Warrior",
    }),
    new EquipmentItem("Holy Symbol", 5, "rare", "defense", {
      classPreference: "Paladin",
    }),
    // Power accessories (Mage preference)
    new EquipmentItem("Arcane Pendant", 4, "uncommon", "power", {
      classPreference: "Mage",
    }),
    new EquipmentItem("Ring of Power", 6, "rare", "power", {
      classPreference: "Mage",
    }),
    new EquipmentItem("Crown of the Ancients", 8, "epic", "power", {
      classPreference: "Mage",
    }),
    new EquipmentItem("Spellweaver's Charm", 5, "uncommon", "power", {
      classPreference: "Mage",
    }),
    // Special accessories
    new EquipmentItem("Blood Amulet", 3, "rare", "power", {
      vampiric: true,
      lifeDrain: 0.15,
      description: "Grants 15% life drain on attacks",
    }),
    new EquipmentItem("Crimson Ring", 2, "uncommon", "agility", {
      vampiric: true,
      lifeDrain: 0.1,
      description: "Grants 10% life drain on attacks",
    }),
    new EquipmentItem("Cursed Obsidian Ring", 8, "epic", "power", {
      cursed: true,
      description: "Cannot be removed once equipped! Drains 2 HP per turn",
      hpDrain: 2,
    }),
    new EquipmentItem("Ring of Eternal Binding", 7, "rare", "agility", {
      cursed: true,
      description: "Cannot be removed once equipped! Drains 3 mana per turn",
      manaDrain: 3,
    }),
  ],
  ...
  genericSpells: {
    "Minor Heal": new Spell(
      "Minor Heal",
      15,
      "heal",
      "Basic healing magic - restores 30 HP",
      2,
      { healAmount: 30 },
    ),
    "Mana Surge": new Spell(
      "Mana Surge",
      10,
      "utility",
      "Channel ambient energy - restore 15 mana",
      3,
      { manaDrain: 15 },
    ),
    Fortify: new Spell(
      ...
      20,
      "buff",
      "Hardens your body - increases defense by 8 for 3 turns",
      4,
      { defenseBoost: 8, duration: 3 },
    ),
    Weaken: new Spell(
      "Weaken",
      20,
      "debuff",
      "Curses enemy - reduces their attack by 25% for 3 turns",
      5,
      { attackReduction: 0.25, duration: 3 },
    ),
    "Greater Heal": new Spell(
      "Greater Heal",
      30,
      "heal",
      "Advanced healing magic - restores 60 HP",
      7,
      { healAmount: 60 },
    ),
    Haste: new Spell(
      ...
      35,
      "buff",
      "Time magic - grants an extra attack each turn for 2 turns",
      9,
      { hasteDuration: 2 },
    ),
  },
  ...
  warriorSpells: {
    "Shield Bash": new Spell(
      "Shield Bash",
      18,
      "damage",
      "Slam with shield - moderate damage, 30% stun chance",
      1,
      {
        ...
        stunChance: 0.3,
        damageType: "physical",
        physicalScaling: "strengthPower",
      },
    ),
    "Battle Stance": new Spell(
      "Battle Stance",
      20,
      "buff",
      "Combat ready - increases strength by 6 for 3 turns",
      2,
      { strengthBoost: 6, duration: 3 },
    ),
    "Shield Wall": new Spell(
      "Shield Wall",
      25,
      "buff",
      "Defensive barrier - reduces next 2 attacks by 20 damage",
      3,
      { defenseBoost: 20, duration: 2 },
    ),
    Warcry: new Spell(
      "Warcry",
      25,
      "debuff",
      "Intimidating roar - reduces enemy attack by 30% for 2 turns",
      4,
      { attackReduction: 0.3, duration: 2 },
    ),
    Cleave: new Spell(
      "Cleave",
      30,
      "damage",
      "Powerful swing - heavy physical damage",
      5,
      {
    ...
    "Iron Skin": new Spell(
      ...
      30,
      "buff",
      "Steel yourself - gain +15 defense for 3 turns",
      6,
      { defenseBoost: 15, duration: 3 },
    ),
    "Berserker Rage": new Spell(
      "Berserker Rage",
      35,
      "buff",
      "Fury unleashed - +10 strength but -5 defense for 3 turns",
      7,
      { strengthBoost: 10, defensePenalty: -5, duration: 3 },
    ),
    Earthquake: new Spell(
      "Earthquake",
      40,
      "damage",
      "Ground slam - massive damage, effective vs constructs",
      8,
      {
        damageMultiplier: 1.5,
        bonusDamage: 12,
        effectiveAgainst: ["Stone Golem", "Gargoyle", "Sunlight Sentinel"],
        damageType: "physical",
        physicalScaling: "strengthPower",
      },
    ),
    "Last Stand": new Spell(
      "Last Stand",
      35,
      "buff",
      "Desperate defense - gain +20 defense for 2 turns",
      9,
      { defenseBoost: 20, duration: 2 },
    ),
    Execute: new Spell(
      ...
      45,
      "damage",
      "Finishing blow - devastating damage when enemy below 50% HP",
      10,
      {
  ...
  mageSpells: {
    "Magic Missile": new Spell(
      "Magic Missile",
      12,
      "damage",
      "Arcane projectile - reliable damage",
      1,
      { damageMultiplier: 1.2, bonusDamage: 0, damageType: "arcane" },
    ),
    "Frost Nova": new Spell(
      "Frost Nova",
      18,
      "damage",
      "Ice blast - effective vs fire enemies",
      2,
      {
        damageMultiplier: 1.2,
        bonusDamage: 2,
        effectiveAgainst: ["Fire Elemental", "Ancient Dragon"],
        weakAgainst: ["Ice Troll"],
        damageType: "ice",
      },
    ),
    Fireball: new Spell(
      ...
      20,
      "damage",
      "Scorching flame - effective vs undead and ice",
      3,
      {
        damageMultiplier: 1.3,
        bonusDamage: 3,
        effectiveAgainst: ["Ice Troll", "Skeleton Warrior", "Undead Knight"],
        weakAgainst: ["Fire Elemental"],
        damageType: "fire",
      },
    ),
    "Lightning Bolt": new Spell(
      "Lightning Bolt",
      22,
      "damage",
      "Electric shock - shatters constructs",
      4,
      {
        damageMultiplier: 1.3,
        bonusDamage: 3,
        effectiveAgainst: ["Stone Golem", "Gargoyle"],
        damageType: "lightning",
      },
    ),
    "Arcane Shield": new Spell(
      "Arcane Shield",
      25,
      "buff",
      "Magical barrier - reduces next 2 attacks by 18 damage",
      5,
      { defenseBoost: 18, duration: 2 },
    ),
    "Ice Storm": new Spell(
      "Ice Storm",
      30,
      "damage",
      "Freezing blizzard - heavy ice damage",
      6,
      {
        damageMultiplier: 1.4,
        bonusDamage: 6,
        effectiveAgainst: ["Fire Elemental", "Ancient Dragon"],
        weakAgainst: ["Ice Troll", "Stone Golem"],
        damageType: "ice",
      },
    ),
    "Chain Lightning": new Spell(
      "Chain Lightning",
      32,
      "damage",
      "Multiple lightning strikes - high damage",
      7,
      {
        damageMultiplier: 1.4,
        bonusDamage: 7,
    ...
    "Meteor Strike": new Spell(
      "Meteor Strike",
      38,
      "damage",
      "Falling meteor - massive fire damage",
      8,
      {
        damageMultiplier: 1.5,
        bonusDamage: 10,
        effectiveAgainst: ["Ice Troll", "Wraith Lord"],
        weakAgainst: ["Fire Elemental", "Ancient Dragon"],
        damageType: "fire",
      },
    ),
    "Time Stop": new Spell(
      ...
      40,
      "debuff",
      "Freezes enemy in time - they skip their next turn",
      9,
      { skipTurns: 1 },
    ),
    "Arcane Apocalypse": new Spell(
      "Arcane Apocalypse",
      50,
      "damage",
      "Ultimate arcane power - pure devastating energy",
      10,
      { damageMultiplier: 1.7, bonusDamage: 15, damageType: "arcane" },
    ),
  },
  ...
  // Rogue spells - Stealth, poisons, and shadow magic
  rogueSpells: {
    "Shadow Meld": new Spell(
      "Shadow Meld",
      15,
      "buff",
      "Merge with shadows - 40% dodge chance for 1 turn",
      1,
      { dodgeChance: 0.4, duration: 1 },
    ),
    "Poison Dagger": new Spell(
      "Poison Dagger",
      15,
      "damage",
      "Venomous strike - initial damage plus 8 poison per turn for 3 turns",
      1,
      {
        damageMultiplier: 1.0,
        bonusDamage: 2,
    ...
    Backstab: new Spell(
      ...
      20,
      "damage",
      "Strike from behind - heavy critical damage",
      3,
      {
        damageMultiplier: 1.5,
        bonusDamage: 5,
    ...
    "Smoke Bomb": new Spell(
      "Smoke Bomb",
      20,
      "debuff",
      "Obscure vision - reduces enemy attack by 35% for 2 turns",
      4,
      { attackReduction: 0.35, duration: 2 },
    ),
    "Deadly Poison": new Spell(
      "Deadly Poison",
      25,
      "damage",
      "Lethal toxin - 12 poison damage per turn for 4 turns",
      5,
      {
        ...
        poisonDamage: 12,
        poisonDuration: 4,
        effectiveAgainst: ["Orc Warrior", "Dark Sorcerer"],
        weakAgainst: ["Stone Golem", "Gargoyle"],
        damageType: "physical",
        physicalScaling: "agility",
      },
    ),
    Evasion: new Spell(
      ...
      28,
      "buff",
      "Perfect dodge - 70% dodge chance for 1 turn",
      6,
      { dodgeChance: 0.7, duration: 1 },
    ),
    "Shadow Strike": new Spell(
      "Shadow Strike",
      30,
      "damage",
      "Attack from darkness - heavy shadow damage",
      7,
      {
        damageMultiplier: 1.5,
        bonusDamage: 8,
        effectiveAgainst: ["Paladin Spirit", "Sunlight Sentinel"],
        damageType: "shadow",
        physicalScaling: "agility",
      },
    ),
    Vanish: new Spell(
      ...
      32,
      "buff",
      "Disappear completely - 80% dodge for 1 turn, +50% damage next attack",
      8,
      { dodgeChance: 0.8, duration: 1, damageBonus: 1.5 },
    ),
    Assassinate: new Spell(
      "Assassinate",
      40,
      "damage",
      "Perfect execution - massive damage when enemy below 40% HP",
      9,
      {
    ...
    "Death Mark": new Spell(
      "Death Mark",
      35,
      "debuff",
      "Mark for death - enemy takes 40% more damage for 3 turns",
      10,
      { vulnerabilityIncrease: 0.4, duration: 3 },
    ),
  },

  // Paladin spells - Holy magic, healing, and divine protection
  paladinSpells: {
    "Holy Strike": new Spell(
      "Holy Strike",
      18,
      "damage",
      "Divine weapon - effective vs undead",
      1,
      {
        damageMultiplier: 1.2,
        bonusDamage: 2,
        effectiveAgainst: ["Skeleton Warrior", "Undead Knight", "Wraith Lord"],
        damageType: "holy",
        physicalScaling: "strengthPower",
      },
    ),
    "Lay on Hands": new Spell(
      "Lay on Hands",
      20,
      "heal",
      "Divine touch - restores 40 HP",
      2,
      { healAmount: 40 },
    ),
    "Divine Shield": new Spell(
      "Divine Shield",
      25,
      "buff",
      "Holy barrier - blocks next 2 attacks for 25 damage each",
      3,
      { defenseBoost: 25, duration: 2 },
    ),
    Smite: new Spell(
      "Smite",
      25,
      "damage",
      "Holy judgement - strong vs undead and demons",
      4,
      {
        damageMultiplier: 1.3,
        bonusDamage: 5,
    ...
    Cleanse: new Spell(
      ...
      22,
      "heal",
      "Purifying light - heals 30 HP and removes debuffs",
      5,
      { healAmount: 30 },
    ),
    "Holy Light": new Spell(
      "Holy Light",
      30,
      "damage",
      "Radiant beam - devastating vs undead",
      6,
      {
        damageMultiplier: 1.4,
        bonusDamage: 8,
        ...
        weakAgainst: ["Paladin Spirit"],
        damageType: "holy",
        physicalScaling: "strengthPower",
      },
    ),
    "Divine Protection": new Spell(
      "Divine Protection",
      30,
      "buff",
      "God's blessing - +12 defense and immune to debuffs for 2 turns",
      7,
      { defenseBoost: 12, duration: 2, debuffImmunity: true },
    ),
    Consecration: new Spell(
      "Consecration",
      35,
      "damage",
      "Holy ground - massive damage vs undead and demons",
      8,
      {
        damageMultiplier: 1.5,
        bonusDamage: 10,
    ...
    Resurrection: new Spell(
      ...
      40,
      "heal",
      "Divine miracle - fully restore HP (once per battle)",
      9,
      { healAmount: 9999, limitedUse: true },
    ),
    "Divine Wrath": new Spell(
      "Divine Wrath",
      45,
      "damage",
      "God's fury - ultimate holy damage",
      10,
      {
  ...
  spells: {
    "Magic Missile": new Spell(
      ...
      15,
      "damage",
      "A basic magical projectile - reliable neutral damage",
      1,
      {
        damageMultiplier: 1.2,
        bonusDamage: 0,
    ...
    Heal: new Spell(
      ...
      20,
      "heal",
      "Restores HP - essential for survival in prolonged battles",
      2,
      { healAmount: 40 },
    ),
    "Fire Bolt": new Spell(
      ...
      20,
      "damage",
      "Burns enemies weak to fire - tactical choice against undead",
      3,
      {
        damageMultiplier: 1.2,
        bonusDamage: 0,
        effectiveAgainst: ["Ice Troll", "Wraith Lord", "Lich King"],
        weakAgainst: ["Fire Elemental", "Ancient Dragon"],
        damageType: "fire",
      },
    ),
    "Mana Drain": new Spell(
      "Mana Drain",
      15,
      "utility",
      "Siphon enemy magical energy - restore 20 mana (no enemy counter)",
      3,
      { manaDrain: 20 },
    ),
    "Ice Shard": new Spell(
      ...
      20,
      "damage",
      "Freezes fire creatures - tactical choice against fire enemies",
      4,
      {
        damageMultiplier: 1.2,
        bonusDamage: 0,
        effectiveAgainst: ["Fire Elemental", "Ancient Dragon", "Demon Lord"],
        weakAgainst: ["Ice Troll", "Stone Golem"],
        damageType: "ice",
      },
    ),
    Fortify: new Spell(
      ...
      20,
      "buff",
      "Hardens your body - increases defense by 10 for 3 turns",
      4,
      { defenseBoost: 10, duration: 3 },
    ),
    Shield: new Spell(
      "Shield",
      25,
      "buff",
      "Creates a magical barrier - reduces next 2 attacks by 15 damage",
      5,
      { defenseBoost: 15, duration: 2 },
    ),
    "Lightning Strike": new Spell(
      "Lightning Strike",
      25,
      "damage",
      "Shatters constructs and armored foes",
      6,
      {
        damageMultiplier: 1.2,
        bonusDamage: 0,
    ...
    Slow: new Spell(
      ...
      18,
      "debuff",
      "Time magic - halves enemy agility for 2 turns (prevents first strike)",
      6,
      { agilityReduction: 0.5, duration: 2 },
    ),
    Weaken: new Spell(
      "Weaken",
      20,
      "debuff",
      "Curses enemy - reduces their attack by 30% for 3 turns",
      7,
      { attackReduction: 0.3, duration: 3 },
    ),
    Cleanse: new Spell(
      "Cleanse",
      15,
      "heal",
      "Purifying light - heals 25 HP and removes all debuffs from you",
      7,
      { healAmount: 25 },
    ),
    "Holy Smite": new Spell(
      "Holy Smite",
      30,
      "damage",
      "Purges undead and shadow creatures with divine wrath",
      8,
      {
        damageMultiplier: 1.2,
        bonusDamage: 0,
    ...
    Vulnerability: new Spell(
      "Vulnerability",
      25,
      "debuff",
      "Hexes enemy - next attack deals 50% more damage",
      9,
      { damageAmplify: 1.5, duration: 1 },
    ),
    Meteor: new Spell(
      "Meteor",
      40,
      "damage",
      "Massive fire damage - devastating but expensive",
      9,
      {
        damageMultiplier: 1.5,
        bonusDamage: 15,
    ...
    "Arcane Blast": new Spell(
      "Arcane Blast",
      35,
      "damage",
      "Pure arcane energy - disrupts magic users",
      10,
      {
        damageMultiplier: 1.2,
        bonusDamage: 0,
        effectiveAgainst: ["Dark Sorcerer"],
        weakAgainst: [],
        damageType: "arcane",
      },
    ),
    "Chrono Lock": new Spell(
      "Chrono Lock",
      35,
      "debuff",
      "Freezes enemy in time - they skip their next turn entirely",
      10,
      { skipTurns: 1 },
    ),
    Haste: new Spell(
    ...
    "Death Bolt": new Spell(
      "Death Bolt",
      22,
      "damage",
      "Necrotic energy bolt - effective against living creatures",
      5,
      {
        damageMultiplier: 1.2,
        bonusDamage: 5,
        ...
        weakAgainst: ["Skeleton Warrior", "Undead Knight", "Lich King"],
        damageType: "necrotic",
      },
    ),
    "Bone Shards": new Spell(
      "Bone Shards",
      28,
      "damage",
      "Launches razor-sharp bone fragments - heavy necrotic damage",
      8,
      {
        damageMultiplier: 1.3,
        bonusDamage: 8,
        effectiveAgainst: ["Dark Sorcerer", "Demon Lord", "Paladin Spirit"],
        weakAgainst: ["Skeleton Warrior", "Undead Knight"],
        damageType: "necrotic",
      },
    ),
    "Plague Touch": new Spell(
      "Plague Touch",
      20,
      "damage",
      "Infects enemy with necrotic plague - deals damage over 3 turns",
      6,
      {
        damageMultiplier: 0.8,
        bonusDamage: 3,
        poisonDamage: 10,
        poisonDuration: 3,
        effectiveAgainst: ["Orc Warrior", "Dark Sorcerer", "Ancient Dragon"],
        weakAgainst: [
          "Skeleton Warrior",
          "Undead Knight",
          "Lich King",
  ...
  vampireSpells: {
    "Blood Bolt": new Spell(
      "Blood Bolt",
      18,
      "damage",
      "Basic vampiric attack - drains 20% HP from damage dealt",
      1,
      {
        damageMultiplier: 1.3,
        bonusDamage: 0,
        lifeDrain: 0.2,
        effectiveAgainst: [],
        weakAgainst: ["Stone Golem", "Gargoyle", "Sunlight Sentinel"],
        damageType: "vampiric",
      },
    ),
    "Crimson Regeneration": new Spell(
      "Crimson Regeneration",
      28,
      "heal",
      "Channel blood magic to heal yourself",
      2,
      { healAmount: 35 },
    ),
    "Bat Swarm": new Spell(
      ...
      25,
      "debuff",
      "Summon bats to obscure enemy vision - reduces attack by 25% for 2 turns",
      3,
      { attackReduction: 0.25, duration: 2 },
    ),
    Hemomancy: new Spell(
      "Hemomancy",
      20,
      "utility",
      "Convert life force to magical energy - restore 25 mana",
      3,
      { manaDrain: 25 },
    ),
    "Shadow Meld": new Spell(
      ...
      28,
      "buff",
      "Merge with shadows - 40% chance to dodge next attack",
      4,
      { dodgeChance: 0.4, duration: 1 },
    ),
    "Blood Siphon": new Spell(
      "Blood Siphon",
      30,
      "damage",
      "Powerful blood drain - 35% life steal, moderate damage",
      5,
      {
        damageMultiplier: 1.4,
        bonusDamage: 5,
        lifeDrain: 0.35,
        effectiveAgainst: [],
        weakAgainst: ["Lich King", "Skeleton Warrior", "Undead Knight"],
        damageType: "vampiric",
      },
    ),
    Nightfall: new Spell(
      ...
      35,
      "buff",
      "Embrace the darkness - gain +12 defense for 3 turns",
      6,
      { defenseBoost: 12, duration: 3 },
    ),
    "Dread Gaze": new Spell(
      "Dread Gaze",
      28,
      "debuff",
      "Paralyzing stare - reduces enemy attack by 35% for 2 turns",
      7,
      { attackReduction: 0.35, duration: 2 },
    ),
    "Corpse Explosion": new Spell(
      "Corpse Explosion",
      40,
      "damage",
      "Detonate dark energy - heavy damage, no life drain",
      8,
      {
        damageMultiplier: 1.5,
        bonusDamage: 10,
        effectiveAgainst: ["Skeleton Warrior", "Undead Knight", "Lich King"],
        weakAgainst: [],
        damageType: "necrotic",
      },
    ),
    "Sanguine Pact": new Spell(
      "Sanguine Pact",
      35,
      "heal",
      "Blood sacrifice - heal 50 HP but take 15 damage first",
      8,
      { healAmount: 50, hpCost: 15 },
    ),
    "Night Terror": new Spell(
      "Night Terror",
      38,
      "damage",
      "Terrifying darkness - 25% life drain, 30% stun chance",
      9,
      {
        damageMultiplier: 1.5,
        bonusDamage: 8,
        lifeDrain: 0.25,
        stunChance: 0.3,
        effectiveAgainst: ["Dark Sorcerer", "Demon Lord"],
        weakAgainst: ["Paladin Spirit"],
        damageType: "vampiric",
      },
    ),
    "Blood Moon": new Spell(
      "Blood Moon",
      45,
      "damage",
      "Ultimate blood magic - 40% life drain, massive damage",
      10,
      {
        damageMultiplier: 1.6,
        bonusDamage: 12,
        lifeDrain: 0.4,
        effectiveAgainst: [],
        weakAgainst: ["Paladin Spirit", "Sunlight Sentinel"],
        damageType: "vampiric",
      },
    ),
    "Vampiric Ascension": new Spell(
      "Vampiric Ascension",
      40,
      "buff",
      "Transcend mortality - gain an extra attack each turn for 2 turns",
      11,
      { hasteDuration: 2 },
    ),
  },
  ...
  enemyTypes: [
    ...
    new Enemy(
      ...
      6,
      10,
      "A vicious green creature with razor-sharp claws",
      0,
      4,
      { physical: 0.9, magical: 1.2 },
      "Goblins are nimble but have no magical defenses",
      { dropPreference: "accessory" },
    ),
    new Enemy(
      "Skeleton Warrior",
      58,
      ...
      3,
      8,
      "An animated skeleton wielding a wickedly sharp blade",
      0,
      3,
      { physical: 1.1, magical: 0.8 },
      "Undead bones are hard to damage with magic but shatter under physical force",
      { dropPreference: "weapon" },
    ),
    new Enemy(
      "Cave Spider",
      48,
      ...
      9,
      18,
      "A massive spider with deadly venom",
      0,
      2,
      { physical: 1.0, magical: 1.3 },
      "Natural creatures are extremely susceptible to magical attacks",
      { dropPreference: "accessory" },
    ),
    new Enemy(
      "Rabid Wolf",
      ...
      8,
      14,
      "A savage wolf with foam-flecked jaws",
      1,
      4,
      { physical: 1.0, magical: 1.2 },
      "Wild beasts have no defense against supernatural forces",
    ),
    ...
    new Enemy(
      ...
      4,
      10,
      "A fierce orc wielding a massive battle axe",
      2,
      6,
      { physical: 0.8, magical: 1.1 },
      "Heavily armored but with no magical protection",
      { dropPreference: "weapon" },
    ),
    new Enemy(
      "Shadow Beast",
      ...
      10,
      24,
      "A creature of pure darkness that strikes without warning",
      2,
      7,
      { physical: 1.3, magical: 0.7 },
      "Shadow creatures are nearly immune to physical attacks but crumble before magical light",
      { dropPreference: "accessory" },
    ),
    new Enemy(
      "Paladin Spirit",
      ...
      5,
      12,
      "The lingering spirit of a holy warrior, radiating divine light",
      3,
      7,
      { physical: 1.0, magical: 0.9 },
      "Holy warriors wield divine power that burns the undead",
      { dropPreference: "weapon", damageType: "holy" },
    ),
    new Enemy(
      "Undead Knight",
      138,
      ...
      2,
      8,
      "A fallen knight in cursed armor, emanating cold death",
      3,
      7,
      { physical: 0.7, magical: 0.9 },
      "Heavy armor protects against magic, but concentrated force can shatter ancient metal",
      { dropPreference: "armor" },
    ),
    new Enemy(
      "Poison Basilisk",
      105,
      ...
      5,
      12,
      "A serpentine monster whose gaze brings paralysis",
      3,
      6,
      { physical: 1.0, magical: 1.2 },
      "Reptilian hide offers little protection against supernatural forces",
      { dropPreference: "accessory" },
    ),
    new Enemy(
      "Minotaur",
      ...
      6,
      14,
      "A bull-headed beast that charges with deadly fury",
      4,
      7,
      { physical: 0.8, magical: 1.3, vampiric: 1.2, necrotic: 1.3 },
      "Massive physical strength but completely vulnerable to magical attacks",
      { dropPreference: "weapon" },
    ),
    new Enemy(
      "Blood Cultist",
      ...
      5,
      16,
      "A deranged worshipper of dark gods, clad in crimson robes",
      4,
      7,
      { physical: 1.1, magical: 0.9, vampiric: 1.4, necrotic: 1.3 },
      "Blood cultists are ironically vulnerable to the very dark powers they worship",
      { dropPreference: "accessory", canCastSpells: true },
    ),
    new Enemy(
      "Gargoyle",
      112,
      ...
      8,
      18,
      "A stone creature that swoops from above with razor claws",
      4,
      6,
      { physical: 0.6, magical: 1.1, vampiric: 0.7, necrotic: 0.8 },
      "Stone skin deflects physical blows but magical energy penetrates deeply",
      { dropPreference: "armor" },
    ),
    ...
    new Enemy(
      ...
      4,
      10,
      "A patchwork abomination of stitched corpses, animated by dark magic",
      5,
      8,
      { physical: 0.9, magical: 1.0, vampiric: 1.5, necrotic: 1.4 },
      "Flesh constructs are particularly susceptible to life-draining and death magic",
      { dropPreference: "weapon" },
    ),
    new Enemy(
      "Fire Elemental",
      ...
      7,
      16,
      "A raging inferno given form, burning everything in its path",
      5,
      8,
      { physical: 1.4, magical: 0.8, vampiric: 0.7, necrotic: 0.8 },
      "Pure elemental fire laughs at physical weapons but succumbs to focused magical energy",
      { dropPreference: "accessory", damageType: "fire" },
    ),
    new Enemy(
      "Ice Troll",
      222,
      ...
      3,
      8,
      "A massive troll encased in magical ice armor",
      5,
      9,
      { physical: 0.7, magical: 0.9, vampiric: 1.2, necrotic: 1.3 },
      "Magical ice provides superior protection, but can be shattered with enough force",
      { dropPreference: "armor" },
    ),
    new Enemy(
      "Holy Inquisitor",
      ...
      5,
      13,
      "A zealot wielding divine fire to purge the unholy",
      5,
      8,
      { physical: 1.0, magical: 0.9, vampiric: 0.5, necrotic: 0.6 },
      "Divine protection shields against vampiric and necrotic corruption",
      { dropPreference: "weapon", damageType: "holy" },
    ),
    new Enemy(
      "Sunlight Sentinel",
      ...
      6,
      14,
      "A crystalline guardian that channels radiant sunlight",
      5,
      8,
      { physical: 0.9, magical: 0.8, vampiric: 0.6, necrotic: 0.7 },
      "Ancient constructs powered by the sun's wrath",
      { dropPreference: "armor", damageType: "light" },
    ),
    new Enemy(
      "Stone Golem",
      248,
      ...
      2,
      8,
      "An unstoppable construct of animated granite",
      6,
      9,
      { physical: 0.5, magical: 1.0, vampiric: 0.6, necrotic: 0.7 },
      "Enchanted stone construction makes it nearly impervious to physical damage",
      { dropPreference: "armor" },
    ),
    new Enemy(
      "Wraith Lord",
      ...
      9,
      28,
      "A spectral tyrant that drains the life from its victims",
      6,
      8,
      { physical: 1.5, magical: 0.6, vampiric: 1.4, necrotic: 1.5 },
      "Spectral form makes physical attacks nearly useless, but magic can bind and destroy spirits",
      { dropPreference: "accessory", canCastSpells: true },
    ),
    ...
    new Enemy(
      ...
      5,
      14,
      "A master of forbidden magic, crackling with dark power",
      7,
      10,
      { physical: 1.1, magical: 0.7, vampiric: 1.3, necrotic: 1.2 },
      "Dark magic shields protect against physical harm but create vulnerability to pure magical force",
      { dropPreference: "accessory", canCastSpells: true },
    ),
    new Enemy(
      "Templar Knight",
      ...
      5,
      12,
      "An elite holy warrior blessed by divine power",
      7,
      10,
      { physical: 0.8, magical: 0.9, vampiric: 0.4, necrotic: 0.5 },
      "Divine blessings make these warriors the bane of the undead and vampires",
      { dropPreference: "weapon", damageType: "holy" },
    ),
    new Enemy(
      "Ancient Dragon",
      ...
      7,
      22,
      "A legendary wyrm whose breath can melt steel",
      8,
      10,
      { physical: 0.6, magical: 0.8, vampiric: 1.2, necrotic: 1.1 },
      "Dragon scales deflect most attacks, but ancient magic provides some protection against spells",
      { dropPreference: "armor", damageType: "fire" },
    ),
    new Enemy(
      "Demon Lord",
      405,
      ...
      6,
      18,
      "An archfiend from the deepest circles of the abyss",
      9,
      10,
      { physical: 0.8, magical: 0.8, vampiric: 1.1, necrotic: 1.0 },
      "Infernal power grants resistance to all mortal attacks, both physical and magical",
      { dropPreference: "weapon" },
    ),
    new Enemy(
      "Lich King",
      ...
      4,
      12,
      "An undead archmage wielding necromantic powers",
      8,
      10,
      { physical: 1.2, magical: 0.6, vampiric: 0.7, necrotic: 0.6 },
      "Undead flesh resists physical damage, but pure magical energy disrupts necromantic bindings",
      { dropPreference: "accessory", canCastSpells: true },
    ),
  ],
  ...
  choiceEvents: [
      ...
      rightOutcome: () => {
        ...
        if (roll < 0.45) {
          ...
          const enemy = {
            ...
            speed: 10,
            agility: 18,
            description: "A starved and enraged beast",
          };
          setTimeout(() => {
    ...
    },
    {
      ...
      leftOutcome: () => {
        ...
        if (roll < 0.4) {
          ...
          if (gameState.character.isVampire) {
            ...
            gameState.character.combatSpecialization.magicalMastery +=
              magicalGain;
            UI.updateDisplay();
      ...
      rightOutcome: () => {
        ...
        if (roll < 0.45) {
          ...
          gameState.character.maxMana = Math.max(
            20,
            gameState.character.maxMana - manaCost,
          );
          gameState.character.mana = Math.min(
    ...
    },
    {
      ...
      rarity: 0.08,
      leftOption: () =>
        gameState.character.isVampire
          ? "Inspect the coffin"
          : "<span style='color: #ff6b6b'>Open the coffin</span> (<i class='fas fa-exclamation-triangle'></i> 30% chance to die!)",
      rightOption: () =>
        gameState.character.isVampire
          ? "Leave it alone"
          : "Leave it sealed (stay safe)",
      leftOutcome: () => {
        if (gameState.character.isVampire) {
        ...
        if (gameState.character.equipment.weapon) {
          const absorbed = Math.ceil(
            gameState.character.equipment.weapon.bonus * 0.25,
          );
        ...
        if (gameState.character.equipment.armor) {
          const absorbed = Math.ceil(
            gameState.character.equipment.armor.bonus * 0.25,
          );
        ...
        if (gameState.character.equipment.accessory) {
          const absorbed = Math.ceil(
            gameState.character.equipment.accessory.bonus * 0.25,
          );
        ...
        const physicalToTransfer = Math.floor(
          gameState.character.combatSpecialization.physicalMastery * 0.01,
        );
        if (physicalToTransfer > 0) {
          gameState.character.combatSpecialization.physicalMastery -=
            physicalToTransfer;
          gameState.character.combatSpecialization.magicalMastery +=
            physicalToTransfer;
        }
        ...
        const hadEquipment =
          gameState.character.equipment.weapon ||
          gameState.character.equipment.armor ||
          gameState.character.equipment.accessory;
        ...
        if (hadEquipment) {
          ...
          message +=
            "<br><span style='color: #888'>Your vampire blood has absorbed a portion of your equipment's power!</span>";
        }
  ...
  riskEvents: [
    ...
    },
    {
      ...
      riskOutcome: () => {
        ...
        const successRate = isVampire ? 0.35 : 0.2;
        const roll = Math.random();
        ...
        if (roll < successRate) {
          ...
          if (gameState.lives <= 0) {
            ...
            gameState.character.hp = Math.floor(
              gameState.character.maxHp * 0.2,
            );
            ...
            if (isVampire) {
              ...
              gameState.character.combatSpecialization.physicalMastery +=
                physicalGain;
              gameState.character.combatSpecialization.magicalMastery +=
                magicalGain + masteryCompensation; // Full magical + half of physical
            } else {
              gameState.character.combatSpecialization.physicalMastery +=
                masteryCompensation;
              gameState.character.combatSpecialization.magicalMastery +=
    ...
    },
    {
      ...
      riskOutcome: () => {
        ...
        if (roll < 0.3) {
          ...
          gameState.character.strength = Math.max(
            5,
            gameState.character.strength - strengthLoss,
          );
          gameState.character.agility = Math.max(
            5,
            gameState.character.agility - agilityLoss,
          );
          gameState.character.takeDamage(damage);
          UI.updateDisplay();
    ...
    },
    {
      ...
      riskOutcome: () => {
        ...
        if (roll < 0.35) {
          ...
          gameState.character.maxMana = Math.max(
            20,
            gameState.character.maxMana - manaLoss,
          );
          gameState.character.mana = Math.min(
            gameState.character.mana,
            gameState.character.maxMana,
  ...
  progressEvents: [
    {
      ...
      effect: () => {
        ...
        gameState.dungeonDepth = Math.min(
          gameState.dungeonDepth + depthGain,
    ...
    },
    {
      ...
      effect: () => {
        ...
        // Cap depth at target depth to ensure final boss encounter
        gameState.dungeonDepth = Math.min(
          gameState.dungeonDepth + progress,
          gameState.targetDepth,
        );
        gameState.addGold(pointReward);
        gameState.addExp(xpReward);
  ...
  getAvailableEnemies(dungeonDepth) {
    ...
    return availableEnemies.length > 0
      ? availableEnemies
      : this.enemyTypes.slice(0, 3);
  },
...
const ClassSelection = {
  ...
  createClassCard: function (className, classInfo) {
    ...
    card.className =
      "backdrop-blur-md bg-gradient-to-br from-[rgba(88,44,131,0.3)] via-[rgba(24,24,32,0.5)] to-[rgba(10,10,20,0.6)] rounded-xl p-5 transition-all duration-300 cursor-pointer shadow-lg transform hover:-translate-y-1";
    ...
    const prefStats = tempInstance.preferredStats
      ? tempInstance.preferredStats.join(", ")
      : "";
  ...
  selectClass: function (className) {
    ...
    document
      .getElementById("game-story")
      .parentElement.classList.remove("hidden");

    UI.addToLog(
      `<span style="color: ${CLASS_DESCRIPTIONS[className].color}">You have chosen the ${className} class!</span>`,
      "levelup",
    );
...
const World = {
  ...
  determineEncounterType() {
    ...
    if (random < weights.combat + weights.item + weights.progress)
      return "progress";
    if (
      random <
      weights.combat + weights.item + weights.progress + weights.choice
    )
      return "choice";
    return "risk";
  },
  ...
  spawnEnemy() {
    ...
    const availableEnemies = GameData.getAvailableEnemies(
      gameState.dungeonDepth,
    );
    const enemyData =
  ...
  createEnemyInstance(enemyData) {
    ...
    gameState.currentEnemy = {
      ...
      takeDamage: function (
        damage,
        damageType = "physical",
        magicalVictory = false,
        physicalVictory = false,
        isCritical = false,
      ) {
        // Apply resistance multiplier based on damage type
        const resistance =
          this.resistances && this.resistances[damageType]
            ? this.resistances[damageType]
            : 1.0;
        ...
        if (this.hp <= 0) {
          ...
          if (magicalVictory || gameState.character.isVampire) {
            ...
            UI.addToLog(
              `Magical mastery increased! Spell effectiveness improved.`,
              "victory",
            );
            ...
            gameState.character.combatSpecialization.physicalMastery += 1;
            UI.addToLog(
              `Physical mastery increased! Physical techniques improved.`,
              "victory",
            );
          } else {
            ...
            gameState.character.combatSpecialization.physicalMastery += 1;
            UI.addToLog(
              `Physical mastery increased! Attack accuracy improved.`,
              "victory",
            );
          }
          ...
          if (gameState.inventory.gems > 0 && Math.random() < 0.2) {
            ...
            gameState.inventory.gems--;
            UI.addToLog(
              `A gem you're carrying attracts extra treasure! +${gemBonus} bonus gold!`,
              "gold",
            );
          }
          ...
          gameState.addExp(xpReward);
          UI.addToLog(
            `${this.name} defeated! +${goldReward} gold, +${xpReward} XP!`,
            "victory",
          );
          UI.updateStory(
            `<i class="fas fa-trophy"></i> Victory! The ${this.name} falls before your might!`,
          );
          ...
          const hasHaste =
            ...
            gameState.character.activeEffects.hasteTurns > 0;
      ...
      performAttack: function () {
        ...
        if (gameState.character.surpriseDefenseActive) {
          UI.addToLog(
            `<i class="fas fa-user-ninja" style="color: #9775fa"></i> <strong>Surprise Defense!</strong> You dodge the attack with ease!`,
            "victory",
          );
          gameState.character.surpriseDefenseActive = false;
          // Apply permanent agility bonus for this fight
          gameState.character.agility +=
            gameState.character.surpriseDefenseBonus;
          UI.addToLog(
            `Your reflexes sharpen! +${gameState.character.surpriseDefenseBonus} Agility for this battle!`,
            "victory",
          );
        ...
        if (
          gameState.character.activeEffects.dodgeChance &&
          gameState.character.activeEffects.dodgeDuration > 0
        ) {
          if (Math.random() < gameState.character.activeEffects.dodgeChance) {
            UI.addToLog(
              `<i class="fas fa-user-ninja"></i> You meld into the shadows and dodge the attack!`,
              "victory",
            );
        ...
        if (this.canCastSpells && Math.random() < 0.4) {
          ...
          const actualDamage = Math.max(
            spellDamage - gameState.character.getMagicalDefense(),
            1,
          );
          const playerDied = gameState.character.takeDamage(
            actualDamage,
            "magical",
          );
          ...
          if (
            gameState.character.characterClass === "Warrior" &&
            gameState.character.battleHardenedTriggered &&
            !gameState.character.battleHardenedAnnounced
          ) {
            UI.addToLog(
              `<i class="fas fa-sword" style="color: #ff6b6b"></i> <strong>Battle Hardened!</strong> Your warrior spirit awakens! +5 Strength, +3 Defense!`,
              "victory",
            );
          ...
          if (
            gameState.character.characterClass === "Paladin" &&
            gameState.character.divineInterventionUsed &&
            !gameState.character.divineInterventionAnnounced
          ) {
            UI.addToLog(
              `<i class="fas fa-shield-halved" style="color: #ffd43b"></i> <strong>Divine Intervention!</strong> Divine power saves you from death!`,
              "victory",
            );
          ...
          if (playerDied && this.hp <= 0) {
            ...
            UI.addToLog(
              `<i class="fas fa-shield-alt" style="color: gold;"></i> You struck the killing blow first! You survive with 1 HP!`,
              "victory",
            );
            UI.addToLog(
              `Narrow victory grants insight! +0.5 to both masteries!`,
              "victory",
            );
        ...
        const actualDamage = Math.max(
          enemyDamage - gameState.character.getPhysicalDefense(),
          1,
        );
    ...
    if (gameState.character.characterClass === "Rogue") {
      if (agilityDifference > 0) {
        ...
        UI.addToLog(
          `<i class="fas fa-user-ninja" style="color: #9775fa"></i> <strong>Surprise Attack!</strong> Your superior agility grants a guaranteed critical hit!`,
          "victory",
        );
        ...
        UI.addToLog(
          `<i class="fas fa-shield-alt" style="color: #9775fa"></i> <strong>Defensive Stance!</strong> You're ready to dodge and gain +3 agility for this fight!`,
          "victory",
        );
      }
    }
    ...
    if (agilityDifference > 0) {
      gameState.currentEnemy.turnsToSkip = Math.min(
        Math.floor(agilityDifference / 10),
        2,
      );
      ...
      if (rogueSurpriseAttack) {
        ...
        UI.addToLog(
          `<i class="fas fa-bolt" style="color: #9775fa"></i> Surprise advantage! You get ${totalTurns} consecutive action(s)!`,
          "victory",
        );
      } else if (gameState.currentEnemy.turnsToSkip > 0) {
        UI.addToLog(
          `<i class="fas fa-bolt" style="color: #43ea7c"></i> Your superior agility grants ${gameState.currentEnemy.turnsToSkip + 1} consecutive action(s) per turn!`,
          "victory",
        );
      }
    } else {
    ...
    const initiativeWarning = enemyGoesFirst
      ? " <i class='fas fa-bolt'></i> The enemy's superior agility allows them to strike first!"
      : "";

    UI.updateStory(
      `<i class="fas fa-swords"></i> Combat! You encounter a ${enemyData.name}! ${enemyData.description}${initiativeWarning}`,
    );
    UI.addToLog(
      `A wild ${enemyData.name} appears!${initiativeWarning}`,
      "combat",
    );
    UI.updateDisplay();
    ...
    if (enemyGoesFirst) {
      ...
      UI.hideAllActionButtons();
      UI.addToLog(
        `The ${enemyData.name} strikes before you can react!`,
        "damage",
      );
      setTimeout(() => {
  ...
  triggerItemEvent() {
    const event =
      GameData.itemEvents[
        Math.floor(Math.random() * GameData.itemEvents.length)
      ];
    const result = event.effect();

    UI.updateStory(
      `<i class="fas fa-search"></i> Discovery: ${event.text} ${result}`,
    );
    UI.addToLog(
      `Discovery #${gameState.discoveries}: ${event.text} ${result}`,
      "event",
    );
  ...
  triggerProgressEvent() {
    ...
    const eventPool =
      availableEvents.length > 0 ? availableEvents : GameData.progressEvents;
    const event = eventPool[Math.floor(Math.random() * eventPool.length)];
    const result = event.effect();

    UI.updateStory(
      `<i class="fas fa-map"></i> Major Discovery: ${event.text} ${result}`,
    );
    UI.addToLog(
      `MAJOR DISCOVERY #${gameState.discoveries}: ${event.text} ${result}`,
      "progress",
    );
  },
  ...
  triggerFinalBoss() {
    ...
    const finalBoss = {
      ...
      description:
        "The ancient master of this cursed dungeon, wreathed in dark magic and wielding powers beyond mortal comprehension",
      resistances: { physical: 0.7, magical: 0.7 }, // 30% damage reduction from both types - fair for both builds
      ...
      takeDamage: function (
        ...
        if (this.hp <= 0) {
          ...
          gameState.addExp(xpReward);
          UI.addToLog(
            `THE DUNGEON LORD FALLS! +${goldReward} gold, +${xpReward} XP!`,
            "victory",
          );
          UI.updateStory(
            `<i class="fas fa-crown"></i> VICTORY! The Dungeon Lord crumbles to dust! You have conquered the depths and emerged as the ultimate champion!`,
          );
          ...
          if (this.hp <= 150 && !this.enraged) {
            ...
            UI.addToLog(
              `The Dungeon Lord enters a berserker rage! Attack increased!`,
              "damage",
            );
          }
      ...
      performSpecialAttack: function () {
        ...
        if (!gameState.inCombat || !gameState.currentEnemy || this.hp <= 0)
          return;
        ...
        if (
          ...
          if (Math.random() < gameState.character.activeEffects.dodgeChance) {
            UI.addToLog(
              `<i class="fas fa-user-ninja"></i> You meld into the shadows and dodge the Dungeon Lord's attack!`,
              "victory",
            );
            gameState.character.activeEffects.dodgeDuration--;
            if (gameState.character.activeEffects.dodgeDuration <= 0) {
        ...
        if (attackType < 0.3) {
          ...
          const actualDamage = Math.max(
            magicDamage - gameState.character.getMagicalDefense(),
            1,
          );
          const playerDied = gameState.character.takeDamage(
            actualDamage,
          ...
          const actualDamage = Math.max(
            drainDamage - gameState.character.getPhysicalDefense(),
            1,
          );
          const playerDied = gameState.character.takeDamage(
            actualDamage,
          ...
          this.hp = Math.min(
            this.hp + Math.floor(actualDamage / 2),
            this.maxHp,
          );
    ...
    UI.updateStory(
      `<i class="fas fa-skull"></i> FINAL BOSS! You have reached the heart of the dungeon! The Dungeon Lord rises from his throne, dark energy crackling around his form! <i class='fas fa-bolt'></i> He strikes first!`,
    );
  ...
  triggerChoiceEvent: function () {
    ...
    const availableEvents = GameData.choiceEvents.filter((event) => {
      if (
        event.minStrength &&
        gameState.character.strength < event.minStrength
      ) {
        return false;
      }
      return true;
    ...
    const eventPool =
      availableEvents.length > 0 ? availableEvents : GameData.choiceEvents;
    const event = eventPool[Math.floor(Math.random() * eventPool.length)];
  ...
  triggerRiskEvent: function () {
    const event =
      GameData.riskEvents[
        Math.floor(Math.random() * GameData.riskEvents.length)
      ];

    UI.updateStory(
      `<i class="fas fa-exclamation-triangle"></i> Risk vs Reward: ${event.text}`,
    );
...
const Equipment = {
  ...
  rollForDrop: function (enemy = null) {
    ...
    if (Math.random() < 0.15) {
      gameState.inventory.manaPotions++;
      UI.addToLog(
        "<i class='fas fa-flask' style='color: #4169E1'></i> Found a <span style='color: #4169E1'>Mana Potion</span>!",
        "victory",
      );
    }
    ...
    const weightedRarities = allowedRarities.flatMap((rarity) =>
      Array(Math.floor(rarityWeights[rarity] * 10)).fill(rarity),
    );
    const targetRarity =
    ...
    const availableItems = equipmentArray.filter(
      (item) =>
        allowedRarities.includes(item.rarity) &&
        item.bonus <= maxBonusByDepth &&
  ...
  offerEquipment: function (type, item) {
    ...
    const statType =
      item.statType || (type === "weapon" ? "strength" : "agility");
    const statName =
      statType === "strength"
        ? "Strength"
        : statType === "power"
    ...
    if (currentItem) {
      const currentStatName =
        currentItem.stat === "strength"
          ? "Strength"
          : currentItem.stat === "power"
            ? "Power"
            : "Agility";
    ...
    setTimeout(() => {
      UI.showChoiceButtons(
        ...
        () => {
          ...
          if (item.cursed) {
            UI.addToLog(
              `<span style="color: #a020f0">The ${item.name} binds to you! You cannot remove it!</span>`,
              "error",
            );
          }
...
const Combat = {
  playerAttack: function () {
    ...
    if (
      gameState.character.characterClass === "Paladin" &&
      gameState.character.divineInterventionHealing
    ) {
      ...
      UI.addToLog(
        `<i class="fas fa-shield-halved" style="color: #ffd43b"></i> <strong>Divine Intervention!</strong> You heal for ${healAmount} HP!`,
        "victory",
      );
    }
    ...
    if (gameState.character.isVampire) {
      ...
      if (!gameState.character.consumeMana(manaCost)) {
        ...
        gameState.character.takeDamage(hpCost);
        UI.addToLog(
          `<span style="color: #ff6b6b"><i class="fas fa-heart-broken"></i> Blood sacrifice! Lost ${hpCost} HP to fuel attack!</span>`,
          "combat",
        );
      }
      ...
      if (agilityHitRoll <= agilityDefense) {
        UI.addToLog(
          `<i class="fas fa-running" style="color: #43ea7c"></i> The ${gameState.currentEnemy.name} swiftly evades your vampiric strike!`,
          "combat",
        );
      ...
      const masteryBonus =
        gameState.character.combatSpecialization.magicalMastery * 1.5; // Changed from physicalMastery
      ...
      UI.addToLog(
        `<i class="fas fa-moon"></i> Vampiric strike drains ${lifeDrain} HP!`,
        "victory",
      );
    } else {
      ...
      const attackingStat = isRogue
        ? gameState.character.getEffectiveAgility()
        : gameState.character.getEffectiveStrength();
      ...
      if (!surpriseAttackActive) {
        ...
        if (defenseHitRoll <= defenseValue) {
          UI.addToLog(
            `<i class="fas fa-shield-alt" style="color: #888"></i> Your attack bounces harmlessly off the ${gameState.currentEnemy.name}'s armor!`,
            "combat",
          );
        ...
        const enemyAgilityValue =
          Math.random() * (gameState.currentEnemy.agility + defenseReduction);

        if (agilityHitRoll <= enemyAgilityValue) {
          UI.addToLog(
            `<i class="fas fa-running" style="color: #43ea7c"></i> The ${gameState.currentEnemy.name} nimbly dodges your attack!`,
            "combat",
          );
      ...
      const baseDamage = isRogue
        ? Math.floor(gameState.character.getEffectiveAgility() * 0.75) + 5
        : gameState.character.getEffectiveStrength();
      const masteryBonus =
        gameState.character.combatSpecialization.physicalMastery * 1.5;
      damage = Math.floor(baseDamage + masteryBonus + Math.random() * 10);

      // Check for vampiric equipment life drain
      if (
        gameState.character.equipment.accessory &&
        gameState.character.equipment.accessory.vampiric
      ) {
        const lifeDrain = Math.floor(
          damage * gameState.character.equipment.accessory.lifeDrain,
        );
        gameState.character.heal(lifeDrain);
        UI.addToLog(
          `<i class="fas fa-tint"></i> ${gameState.character.equipment.accessory.name} drains ${lifeDrain} HP!`,
          "victory",
        );
      }
    }
    ...
    if (!gameState.character.isVampire) {
      ...
      if (gameState.character.surpriseAttackReady) {
        ...
        UI.addToLog(
          `<span style='color: #9775fa'><i class="fas fa-star"></i> SURPRISE CRITICAL! You strike from the shadows!</span>`,
          "combat",
        );
      } else {
    ...
    if (gameState.character.activeEffects.vulnerableNext) {
      damage = Math.floor(
        damage * gameState.character.activeEffects.vulnerableNext,
      );
      gameState.character.activeEffects.vulnerableNext = false;
      UI.addToLog(
        `<span style='color: #ff6b6b'>VULNERABILITY TRIGGERED!</span>`,
        "combat",
      );
    }
    ...
    gameState.currentEnemy.takeDamage(
      damage,
      damageType,
      isMagicalVictory,
      isPhysicalVictory,
    ...
    if (gameState.currentEnemy && gameState.currentEnemy.turnsToSkip > 0) {
      ...
      if (gameState.currentEnemy.turnsToSkip > 0) {
        UI.addToLog(
          `<i class="fas fa-bolt" style="color: #43ea7c"></i> Your superior agility allows another attack! (${gameState.currentEnemy.turnsToSkip} turn(s) remaining)`,
          "victory",
        );
      }
    ...
    if (gameState.character.activeEffects.hasteTurns > 0) {
      ...
      if (gameState.character.activeEffects.hasteTurns > 0) {
        UI.addToLog(
          `<i class="fas fa-bolt" style="color: #ffd700"></i> Haste grants you another action! (${gameState.character.activeEffects.hasteTurns} turn(s) remaining)`,
          "victory",
        );
      } else {
        UI.addToLog(
          `<i class="fas fa-hourglass-end" style="color: #aaa"></i> Haste fades...`,
          "combat",
        );
      }
  ...
  playerCastSpell: function () {
    ...
    const spell = GameData.getSpell(
      gameState.character.selectedSpell,
      gameState.character.isVampire,
    );
    if (!spell) {
    ...
    if (gameState.character.characterClass === "Mage") {
      gameState.character.spellComboCount =
        (gameState.character.spellComboCount || 0) + 1;
      if (gameState.character.spellComboCount >= 3) {
        arcaneSurgeActive = true;
        gameState.character.spellComboCount = 0;
        UI.addToLog(
          `<i class="fas fa-magic" style="color: #4dabf7"></i> <strong>Arcane Surge!</strong> Your next spell is empowered! (+50% damage, -50% mana cost)`,
          "victory",
        );
      }
    }
    ...
    if (!gameState.character.consumeMana(manaCost)) {
      if (gameState.character.isVampire) {
        ...
        gameState.character.takeDamage(hpCost);
        UI.addToLog(
          `<span style="color: #ff6b6b"><i class="fas fa-heart-broken"></i> Blood sacrifice! Lost ${hpCost} HP to cast ${spell.name}!</span>`,
          "combat",
        );
      } else {
        UI.addToLog(
          `Not enough mana to cast ${spell.name}! (Requires ${spell.manaCost} mana)`,
          "error",
        );
        return;
      }
    ...
    if (spell.spellType === "utility") {
      ...
      if (spell.name === "Mana Drain") {
        ...
        UI.addToLog(
          `<i class="fas fa-magic"></i> ${spell.name} siphons magical energy! Restored ${manaRestored} mana!`,
          "victory",
        );
        UI.updateDisplay();
        ...
        gameState.character.restoreMana(manaRestored);
        UI.addToLog(
          `<i class="fas fa-tint"></i> ${spell.name} converts blood to mana! Restored ${manaRestored} mana!`,
          "victory",
        );
        UI.updateDisplay();
        // No enemy counter for utility spells
    ...
    if (spell.spellType === "heal") {
      ...
      if (spell.name === "Sanguine Pact" && spell.hpCost) {
        ...
        UI.addToLog(
          `<i class="fas fa-skull"></i> ${spell.name} requires a blood sacrifice! Lost ${spell.hpCost} HP!`,
          "combat",
        );
      }
      ...
      if (spell.name === "Cleanse") {
        ...
        UI.addToLog(
          `<i class="fas fa-sun"></i> ${spell.name} purifies you! Restored ${healAmount} HP and removed debuffs!`,
          "victory",
        );
      } else if (spell.name === "Crimson Regeneration") {
        UI.addToLog(
          `<i class="fas fa-heart"></i> ${spell.name} channels vampiric essence! Restored ${healAmount} HP!`,
          "victory",
        );
      } else if (spell.name === "Sanguine Pact") {
        UI.addToLog(
          `<i class="fas fa-heart"></i> ${spell.name} completes the ritual! Restored ${healAmount} HP!`,
          "victory",
        );
      } else {
        UI.addToLog(
          `<i class="fas fa-heart"></i> ${spell.name} restores ${healAmount} HP!`,
          "victory",
        );
      }
      UI.updateDisplay();
    ...
    if (spell.spellType === "buff") {
      ...
      if (spell.name === "Shield") {
        ...
        UI.addToLog(
          `<i class="fas fa-shield-alt"></i> ${spell.name} creates a protective barrier! (Blocks ${spell.defenseBoost} damage for ${spell.duration} turns)`,
          "victory",
        );
        ...
        UI.addToLog(
          `<i class="fas fa-fist-raised"></i> ${spell.name} hardens your body! (+${spell.defenseBoost} defense for ${spell.duration} turns)`,
          "victory",
        );
        ...
        gameState.character.activeEffects.fortifyAmount = spell.defenseBoost;
        UI.addToLog(
          `<i class="fas fa-moon"></i> ${spell.name} embraces the darkness! (+${spell.defenseBoost} defense for ${spell.duration} turns)`,
          "victory",
        );
      } else if (spell.name === "Haste") {
        gameState.character.activeEffects.hasteTurns = spell.hasteDuration;
        UI.addToLog(
          `<i class="fas fa-bolt"></i> ${spell.name} grants supernatural speed! You gain an extra attack for ${spell.hasteDuration} turns!`,
          "victory",
        );
      } else if (spell.name === "Vampiric Ascension") {
        gameState.character.activeEffects.hasteTurns = spell.hasteDuration;
        UI.addToLog(
          `<i class="fas fa-dragon"></i> ${spell.name} transcends mortality! You gain an extra attack for ${spell.hasteDuration} turns!`,
          "victory",
        );
        ...
        UI.addToLog(
          `<i class="fas fa-user-ninja"></i> ${spell.name} lets you blend into shadows! ${Math.floor(spell.dodgeChance * 100)}% chance to dodge next attack!`,
          "victory",
        );
      }
      UI.updateDisplay();
    ...
    if (spell.spellType === "debuff") {
      ...
      if (spell.name === "Weaken") {
        ...
        UI.addToLog(
          `<i class="fas fa-skull-crossbones"></i> ${spell.name} curses the enemy! Attack reduced by ${Math.floor(spell.attackReduction * 100)}% for ${spell.duration} turns!`,
          "combat",
        );
        ...
        const playerAgility = gameState.character.getEffectiveAgility();
        const enemyAgilityAfterSlow = Math.floor(
          gameState.currentEnemy.agility * (1 - spell.agilityReduction),
        );
        const newAgilityDifference = playerAgility - enemyAgilityAfterSlow;

        // Update turnsToSkip with new advantage (capped at 2)
        const newTurnsToSkip =
          newAgilityDifference > 0
            ? Math.min(Math.floor(newAgilityDifference / 10), 2)
            : 0;
        ...
        if (turnsGained > 0) {
          UI.addToLog(
            `<i class="fas fa-hourglass-half"></i> ${spell.name} warps time! Agility halved for ${spell.duration} turns! <span style="color: #43ea7c">Gained ${turnsGained} additional turn(s)!</span>`,
            "combat",
          );
        } else {
          UI.addToLog(
            `<i class="fas fa-hourglass-half"></i> ${spell.name} warps time! Agility halved for ${spell.duration} turns!`,
            "combat",
          );
        }
        ...
        UI.addToLog(
          `<i class="fas fa-crosshairs"></i> ${spell.name} hexes the enemy! Your next attack will deal ${Math.floor((spell.damageAmplify - 1) * 100)}% more damage!`,
          "combat",
        );
      } else if (spell.name === "Chrono Lock") {
        gameState.currentEnemy.chronoLocked = spell.skipTurns;
        UI.addToLog(
          `<i class="fas fa-clock"></i> ${spell.name} freezes the enemy in time! They will skip their next turn!`,
          "combat",
        );
        ...
        gameState.currentEnemy.weakenAmount = spell.attackReduction;
        UI.addToLog(
          `<i class="fas fa-crow"></i> ${spell.name} obscures the enemy's vision! Attack reduced by ${Math.floor(spell.attackReduction * 100)}% for ${spell.duration} turns!`,
          "combat",
        );
        ...
        gameState.currentEnemy.weakenAmount = spell.attackReduction;
        UI.addToLog(
          `<i class="fas fa-eye"></i> ${spell.name} paralyzes the enemy with fear! Attack reduced by ${Math.floor(spell.attackReduction * 100)}% for ${spell.duration} turns!`,
          "combat",
        );
      }
      UI.updateDisplay();
      ...
      if (
        ...
        gameState.currentEnemy.chronoLocked > 0
      ) {
        gameState.currentEnemy.chronoLocked--;
    ...
    if (spell.physicalScaling === "strengthPower") {
      ...
      masteryBonus =
        gameState.character.combatSpecialization.magicalMastery * 1.5;
    ...
    if (spell.physicalScaling) {
      ...
      if (defenseHitRoll <= defenseValue) {
        UI.addToLog(
          `<i class="fas fa-shield-alt" style="color: #888"></i> ${spell.name} glances off the ${gameState.currentEnemy.name}'s armor!`,
          "combat",
        );
        setTimeout(() => {
          if (gameState.inCombat && !gameState.isGameOver) {
      ...
      if (agilityHitRoll <= agilityDefense) {
        UI.addToLog(
          `<i class="fas fa-running" style="color: #43ea7c"></i> The ${gameState.currentEnemy.name} evades ${spell.name}!`,
          "combat",
        );
        setTimeout(() => {
          if (gameState.inCombat && !gameState.isGameOver) {
      ...
      if (agilityHitRoll <= agilityDefense) {
        UI.addToLog(
          `<i class="fas fa-running" style="color: #43ea7c"></i> The ${gameState.currentEnemy.name} dodges ${spell.name}!`,
          "combat",
        );
        setTimeout(() => {
          if (gameState.inCombat && !gameState.isGameOver) {
    ...
    let totalDamage = Math.floor(
      baseDamage * spell.damageMultiplier +
        masteryBonus +
        spell.bonusDamage +
        Math.random() * 10,
    );
    ...
    if (gameState.character.activeEffects.vulnerableNext) {
      totalDamage = Math.floor(
        totalDamage * gameState.character.activeEffects.vulnerableNext,
      );
      gameState.character.activeEffects.vulnerableNext = false;
      UI.addToLog(
        `<span style='color: #ff6b6b'>VULNERABILITY TRIGGERED!</span>`,
    ...
    if (spell.multiHit) {
      ...
      UI.addToLog(
        `<i class="fas fa-magic"></i> ${spell.name} strikes ${hits} times!`,
        "combat",
      );
      for (let i = 0; i < hits; i++) {
        gameState.currentEnemy.takeDamage(
          Math.floor(totalDamage + Math.random() * 5),
    ...
    if (spell.effectiveAgainst && spell.effectiveAgainst.includes(enemyName)) {
      ...
      effectivenessMessage =
        " <span style='color: #43ea7c'>SUPER EFFECTIVE!</span>";
    } else if (spell.weakAgainst && spell.weakAgainst.includes(enemyName)) {
      totalDamage = Math.floor(totalDamage * 0.5);
      effectivenessMessage =
        " <span style='color: #ff6b6b'>Not very effective...</span>";
    }

    if (!spell.multiHit) {
      UI.addToLog(
        `<i class="fas fa-magic"></i> You cast ${spell.name}!${effectivenessMessage}`,
        "combat",
      );
    ...
    if (spell.lifeDrain && gameState.character.isVampire) {
      ...
      gameState.character.heal(lifeDrain);
      UI.addToLog(
        `<i class="fas fa-tint"></i> Drained ${lifeDrain} HP from the enemy!`,
        "victory",
      );
    }
    ...
    if (spell.stunChance && Math.random() < spell.stunChance) {
      if (gameState.currentEnemy) {
        ...
        UI.addToLog(
          `<i class="fas fa-moon"></i> ${spell.name} terrifies the enemy! They are stunned and cannot attack this turn!`,
          "victory",
        );
      }
    }
    ...
    if (gameState.currentEnemy && gameState.currentEnemy.stunned) {
  ...
  attemptEscape: function () {
    ...
    UI.updateStory(
      "<i class='fas fa-running'></i> You manage to escape from the enemy! You live to fight another day.",
    );
  ...
  enemyAttack: function () {
    ...
    if (
      gameState.character.equipment.accessory &&
      gameState.character.equipment.accessory.cursed
    ) {
      const cursedItem = gameState.character.equipment.accessory;
      if (cursedItem.hpDrain) {
        const died = gameState.character.takeDamage(
          cursedItem.hpDrain,
          "cursed",
        );
        UI.addToLog(
          `<span style="color: #a020f0">${cursedItem.name} drains ${cursedItem.hpDrain} HP!</span>`,
          "error",
        );
        if (died) {
      ...
      if (cursedItem.manaDrain) {
        gameState.character.mana = Math.max(
          0,
          gameState.character.mana - cursedItem.manaDrain,
        );
        UI.addToLog(
          `<span style="color: #a020f0">${cursedItem.name} drains ${cursedItem.manaDrain} mana!</span>`,
          "error",
        );
        UI.updateDisplay();
      }
  ...
  endBattle: function () {
    ...
    if (
      gameState.character.characterClass === "Warrior" &&
      gameState.character.battleHardenedBonus
    ) {
      gameState.character.strength -=
        gameState.character.battleHardenedBonus.strength;
      gameState.character.defense -=
        gameState.character.battleHardenedBonus.defense;
      gameState.character.battleHardenedBonus = null;
    }

    // Reset Rogue's surprise defense agility bonus
    if (
      gameState.character.characterClass === "Rogue" &&
      gameState.character.surpriseDefenseBonus > 0
    ) {
...
const UI = {
  ...
  initialize: function () {
    ...
    this.addToLog(
      "Welcome to Dungeon Crawler! Click 'Start New Game' to begin your journey!",
      "start",
    );
  },
  ...
  updateDisplay: function () {
    ...
    if (this.elements.exp && this.elements.expNeeded) {
      ...
      this.elements.expNeeded.textContent = gameState.getExpForLevel(
        gameState.level + 1,
      );
    }
    ...
    if (gameState.character) {
      ...
      this.elements.playerDefense.textContent = Math.floor(
        gameState.character.getPhysicalDefense(),
      );
      this.elements.playerPower.textContent = gameState.character.power;

      // Update mastery displays
      if (this.elements.physicalMastery) {
        this.elements.physicalMastery.textContent =
          gameState.character.combatSpecialization.physicalMastery;
      }
      if (this.elements.magicalMastery) {
        this.elements.magicalMastery.textContent =
          gameState.character.combatSpecialization.magicalMastery;
      }
      ...
      const manaPercent =
        (gameState.character.mana / gameState.character.maxMana) * 100;
    ...
    if (gameState.character) {
      if (weaponSlot) {
        ...
        weaponSlot.textContent = weapon
          ? `Weapon: ${weapon.name} (+${weapon.bonus})`
          : "Weapon: None";
        weaponSlot.className = weapon
          ? "text-xs text-yellow-300"
          : "text-xs text-gray-400";
      }
      if (armorSlot) {
        const armor = gameState.character.equipment.armor;
        armorSlot.textContent = armor
          ? `Armor: ${armor.name} (+${armor.bonus})`
          : "Armor: None";
        armorSlot.className = armor
          ? "text-xs text-yellow-300"
          : "text-xs text-gray-400";
      }
      if (accessorySlot) {
        const accessory = gameState.character.equipment.accessory;
        accessorySlot.textContent = accessory
          ? `Accessory: ${accessory.name} (+${accessory.bonus})`
          : "Accessory: None";
        accessorySlot.className = accessory
          ? "text-xs text-yellow-300"
          : "text-xs text-gray-400";
      }
      if (vampireStatus) {
        vampireStatus.classList.toggle(
          "hidden",
          !gameState.character.isVampire,
        );
        vampireStatus.style.display = gameState.character.isVampire
          ? "block"
          : "none";
      }
      if (playerClassEl) {
      ...
      if (vampireStatus) {
        vampireStatus.classList.add("hidden");
    ...
    if (this.elements.explorationPercent && this.elements.explorationBar) {
      const explorationPercent = Math.floor(
        (gameState.dungeonDepth / gameState.targetDepth) * 100,
      );
    ...
    if (this.elements.floorName) {
      ...
      this.elements.floorName.textContent =
        floorNames[gameState.dungeonDepth] || "Unknown";
    }
    ...
    if (gameState.inCombat && gameState.currentEnemy) {
      ...
      this.elements.enemyHp.textContent = Math.max(
        0,
    ...
    if (!playerStatusElement) {
      ...
      hpBarParent.parentElement.insertBefore(
        playerStatusElement,
        hpBarParent.nextSibling,
      );
    }
  ...
  addToLog: function (message, type = "normal") {
    ...
    // Ensure combat log auto-scrolls to bottom (scroll the panel container)
    const logPanel = document.querySelector(".panel.panel-log");
    if (logPanel) {
      logPanel.scrollTop = logPanel.scrollHeight;
    } else {
      // Fallback to inner log div
  ...
  showExplorationButtons: function () {
    ...
    if (manaPotionBtn) {
      const showManaPotion =
        gameState.inventory.manaPotions > 0 &&
        gameState.character.mana < gameState.character.maxMana;
  ...
  showSpellSelectionButtons: function () {
    ...
    gameState.character.knownSpells.forEach((spellName) => {
      ...
      button.className =
        "spell-choice-btn bg-gradient-to-r from-purple-600 to-purple-500 text-white px-4 py-3 rounded-lg font-bold transition-all hover:from-purple-500 hover:to-purple-400 hover:-translate-y-0.5 hover:shadow-lg";
      // Add inline styles with !important to ensure background shows
      button.style.cssText =
        "background: linear-gradient(to right, rgb(147, 51, 234), rgb(168, 85, 247)) !important; color: white !important;";
      button.onclick = () => {
      ...
      if (gameState.currentEnemy) {
        if (
          spell.effectiveAgainst &&
          spell.effectiveAgainst.includes(gameState.currentEnemy.name)
        ) {
          effectivenessIcon =
            ' <i class="fas fa-bolt" style="color: #43ea7c;"></i>';
        } else if (
          spell.weakAgainst &&
          spell.weakAgainst.includes(gameState.currentEnemy.name)
        ) {
          effectivenessIcon =
    ...
    cancelBtn.className =
      "spell-choice-btn bg-gradient-to-r from-gray-600 to-gray-500 text-white px-4 py-3 rounded-lg font-bold transition-all hover:from-gray-500 hover:to-gray-400 hover:-translate-y-0.5 hover:shadow-lg";
    cancelBtn.onclick = () => {
      this.hideSpellButtons();
      this.showCombatButtons();
    };
  ...
  showChoiceButtons: function (
    ...
    actionsDiv.appendChild(
      UI.createButton(
        ...
        "blue",
        "blue",
        "choice-btn left-choice",
        () => handleOutcome(leftOutcome(), "arrow-left"),
      ),
    );
    actionsDiv.appendChild(
      UI.createButton(
        ...
        "green",
        "green",
        "choice-btn right-choice",
        () => handleOutcome(rightOutcome(), "arrow-right"),
      ),
    );
  },
  ...
  showRiskButtons: function (riskOption, safeOption, riskOutcome, safeOutcome) {
    ...
    actionsDiv.appendChild(
      UI.createButton(safeOption, "cyan", "cyan", "risk-btn safe", () => {
  ...
  removeChoiceButtons: function () {
    ...
    document
      .querySelectorAll(
        ".choice-btn, .risk-btn, .spell-choice-btn, .vampire-spell-btn, #confirm-vampire-spells-btn",
      )
      .forEach((btn) => btn.remove());
  },

  showSpellChoiceUI: function (spellChoices) {
    console.log("showSpellChoiceUI called with spellChoices:", spellChoices);
    // Hide normal buttons
    this.hideAllActionButtons();
    // Guard against multiple concurrent choice UIs
    if (this._spellChoiceActive) {
      console.log("Spell choice UI already active, returning");
      return;
    }
    this._spellChoiceActive = true;
    console.log("Setting _spellChoiceActive to true");
    ...
    // Reset flag on error or if actionsDiv doesn't exist
    if (!actionsDiv) {
      console.error("Could not find actions container for spell choices");
      this._spellChoiceActive = false;
      return;
    }

    console.log(
      "actionsDiv found, creating buttons for",
      spellChoices.length,
      "spells",
    );

    // Ensure parent container is visible - aggressive display settings
    actionsDiv.classList.remove("hidden");
    actionsDiv.style.display = "flex";
    actionsDiv.style.visibility = "visible";
    actionsDiv.style.opacity = "1";
    actionsDiv.style.pointerEvents = "auto";
    actionsDiv.style.zIndex = "2000";
    actionsDiv.style.position = "relative";
    actionsDiv.style.flexWrap = "wrap";
    actionsDiv.style.alignItems = "stretch";

    // Also ensure the controls container is visible
    ...
    const disableAndRemoveAllChoices = () => {
      // Aggressively remove any buttons we may have appended in the controls area
      const controls = document.getElementById("controls");
      const removeBtn = (btn) => {
        btn.disabled = true;
        btn.remove();
      ...
      if (controls) {
        // Remove common choice buttons within controls
        controls
          .querySelectorAll(
            ".spell-choice-btn, .choice-btn, .risk-btn, #confirm-vampire-spells-btn",
          )
          .forEach(removeBtn);
        // Also remove any buttons inside potential wrappers
        controls
          .querySelectorAll(
            "#spell-choice, .spell-choice, .spell-choice-container, .replace-choice",
          )
          .forEach((container) => {
            container.querySelectorAll("button").forEach(removeBtn);
            container.remove();
          });
        // Final wipe of controls container to guarantee removal
        controls.querySelectorAll("button").forEach((btn) => {
          if (
            !btn.id ||
            btn.classList.contains("spell-choice-btn") ||
            btn.classList.contains("choice-btn") ||
            btn.classList.contains("risk-btn")
          ) {
            // keep primary controls by ID; remove any ad-hoc buttons
            if (
              ![
                "explore-btn",
                "rest-btn",
      ...
      // Global fallback queries across the document
      document
        .querySelectorAll(
          ".spell-choice-btn, .choice-btn, .risk-btn, #confirm-vampire-spells-btn",
        )
      ...
      document
        ...
        .forEach((el) => {
          el.querySelectorAll("button").forEach(removeBtn);
          el.remove();
        });
      // Ensure cleanup even if something was queued to render; run again next tick
      setTimeout(() => {
        document
          .querySelectorAll(
            ".spell-choice-btn, .choice-btn, .risk-btn, #confirm-vampire-spells-btn",
        ...
        const actionsDiv = UI.elements.exploreBtn?.parentElement;
        if (actionsDiv) {
          // Remove any remaining ad-hoc choice buttons appended directly
          actionsDiv
            .querySelectorAll(
              ".spell-choice-btn, .choice-btn, .risk-btn, #confirm-vampire-spells-btn",
            )
    ...
    spellChoices.forEach((spellName, index) => {
      ...
      if (!spell) {
        console.error(
          `Failed to find spell: ${spellName}, isVampire: ${gameState.character.isVampire}`,
        );
        return; // Skip this spell if not found
      }
      console.log(`Creating spell choice button for: ${spellName}`);
      const btn = document.createElement("button");
      btn.innerHTML = `<i class="fas fa-wand-magic"></i> ${spellName}<br><small>${spell.description}</small>`;
      btn.className =
        "spell-choice-btn bg-gradient-to-r from-purple-600 to-purple-500 text-white px-4 py-3 rounded-lg font-bold";
      ...
      btn.onclick = () => {
        // Disable this button immediately to prevent duplicate clicks
        btn.disabled = true;
        const needsReplacement = gameState.learnSpell(spellName);
        // If replacement UI is needed, do NOT clear buttons here; let showSpellReplaceUI render
        if (needsReplacement) {
          return;
        }
        // Otherwise, clean up and return to exploration (when not in combat)
        disableAndRemoveAllChoices();
        if (!gameState.inCombat && !gameState.isGameOver) {
      ...
      console.log(
        `Appended spell choice button for: ${spellName}, total buttons: ${actionsDiv.children.length}, button visibility: ${btn.style.visibility}, opacity: ${btn.style.opacity}, display: ${btn.style.display}`,
      );
    });
    ...
    skipBtn.className =
      "spell-choice-btn bg-gradient-to-r from-gray-500 to-gray-400 text-white px-4 py-3 rounded-lg font-bold";
    ...
    skipBtn.onclick = () => {
      console.log("Skip spell button clicked");
    ...
    console.log("Skip button appended");
    console.log("=== Spell Choice UI Complete ===");
    console.log("Actions div display:", actionsDiv.style.display);
    console.log("Actions div visibility:", actionsDiv.style.visibility);
    console.log("Actions div opacity:", actionsDiv.style.opacity);
    console.log("Total children in actions div:", actionsDiv.children.length);
  },
  ...
  showVampireSpellSelectionUI: function () {
    ...
    availableSpells.forEach((spellName) => {
      ...
      btn.innerHTML = `<i class="fas fa-wand-magic"></i> ${spellName}<br><small>${spell.description}</small>`;
      btn.className =
        "bg-gradient-to-r from-gray-600 to-gray-500 text-white px-4 py-3 rounded-lg font-bold transition-all hover:from-gray-700 hover:to-gray-600 hover:-translate-y-0.5 hover:shadow-lg vampire-spell-btn w-full mb-2";
      ...
      btn.onclick = () => {
        ...
        if (selectedSpells.includes(spellName)) {
          ...
          btn.className =
            "bg-gradient-to-r from-gray-600 to-gray-500 text-white px-4 py-3 rounded-lg font-bold transition-all hover:from-gray-700 hover:to-gray-600 hover:-translate-y-0.5 hover:shadow-lg";
          btn.innerHTML = `<i class="fas fa-wand-magic"></i> ${spellName}<br><small>${spell.description}</small>`;
        } else {
          ...
          if (selectedSpells.length < 4) {
            ...
            btn.className =
              "bg-gradient-to-r from-red-700 to-red-600 text-white px-4 py-3 rounded-lg font-bold transition-all hover:from-red-800 hover:to-red-700 hover:-translate-y-0.5 hover:shadow-lg vampire-spell-btn w-full mb-2 border-4 border-yellow-400";
        ...
        if (selectedSpells.length > 0 && selectedSpells.length <= 4) {
          if (!confirmBtn) {
            ...
            confirmBtn.className =
              "bg-gradient-to-r from-purple-600 to-purple-500 text-white px-4 py-3 rounded-lg font-bold transition-all hover:from-purple-700 hover:to-purple-600 hover:-translate-y-0.5 hover:shadow-lg w-full mb-2 mt-4";
            confirmBtn.onclick = function () {
              gameState.character.knownSpells = [...selectedSpells];
              gameState.character.selectedSpell = selectedSpells[0];
              UI.addToLog(
                `<i class="fas fa-moon"></i> Vampire powers awakened: ${selectedSpells.join(", ")}!`,
                "victory",
              );
              UI.removeChoiceButtons();
              if (!gameState.inCombat && !gameState.isGameOver) {
    ...
    skipBtn.className =
      "bg-gradient-to-r from-gray-500 to-gray-400 text-white px-4 py-3 rounded-lg font-bold transition-all hover:from-gray-600 hover:to-gray-500 hover:-translate-y-0.5 hover:shadow-lg spell-choice-btn w-full mb-2";
    skipBtn.onclick = () => {
  ...
  showSpellReplaceUI: function (newSpell) {
    // Hide normal buttons (force allows replacement UI to render even if spell-choice active)
    this.hideAllActionButtons(true);
    // Mark active so other hides are skipped during replace UI
    this._spellChoiceActive = true;

    const newSpellData = GameData.getSpell(
      newSpell,
      gameState.character.isVampire,
    );
    if (!newSpellData) {
      console.error(
        `Failed to find new spell: ${newSpell}, isVampire: ${gameState.character.isVampire}`,
      );
    ...
    const actionsDiv = this.elements.exploreBtn.parentElement;
    if (!actionsDiv) {
      console.error("Could not find actions container for spell replace UI");
      this._spellChoiceActive = false;
      return;
    }
    actionsDiv.classList.remove("hidden");
    ...
    gameState.character.knownSpells.forEach((spellName) => {
      // Do not offer replacing the same spell with itself
      if (spellName === newSpell) return;
      const spell = GameData.getSpell(spellName, gameState.character.isVampire);
      if (!spell) {
        console.error(
          `Failed to find spell: ${spellName}, isVampire: ${gameState.character.isVampire}`,
      ...
      btn.className =
        "spell-choice-btn bg-gradient-to-r from-red-500 to-red-400 text-white px-4 py-3 rounded-lg font-bold";
      btn.style.display = "block";
    ...
    skipBtn.style.pointerEvents = "auto";
    skipBtn.style.boxShadow = "none";
    skipBtn.style.textShadow = "none";
    skipBtn.style.filter = "none";
    skipBtn.onclick = () => {
      UI.addToLog(`Decided not to learn ${newSpell}.`, "info");
      this.removeChoiceButtons();
      this._spellChoiceActive = false;
  ...
  showVictoryShareButton: function () {
    ...
    shareBtn.className =
      "bg-gradient-to-r from-yellow-500 to-yellow-400 text-white px-5 py-3 rounded-full font-bold transition-all hover:from-yellow-600 hover:to-yellow-500 hover:-translate-y-0.5 hover:shadow-lg disabled:bg-gray-600 disabled:cursor-not-allowed disabled:transform-none victory-share-btn";
    shareBtn.onclick = () => {
      this.shareVictory();
    };
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard
        ...
        .then(() => {
          ...
          UI.addToLog(
            "Paste anywhere to share your achievement! 🎊",
            "victory",
          );
        })
        .catch(() => {
  ...
  hideAllActionButtons: function (force = false) {
    // If a spell choice/replace UI is active, do NOT hide controls unless forced
    if (this._spellChoiceActive && !force) {
      console.log(
        "hideAllActionButtons skipped because _spellChoiceActive is true",
      );
      return;
    }
    ...
    this.removeChoiceButtons();
    document
      .querySelectorAll(".victory-share-btn")
      .forEach((btn) => btn.remove());

    // Reset spell choice flag when hiding all buttons
    this._spellChoiceActive = false;
  },
...
const Shop = {
  upgrades: {
    ...
    mastery: {
      cost: 300,
      benefit: "+1 to both masteries",
      icon: "fas fa-shield-alt",
      baseCost: 300,
    },
    potion: {
      cost: 100,
      benefit: "Buy 1 Healing Potion",
      icon: "fas fa-flask",
      baseCost: 100,
    },
    manaPotion: {
      cost: 80,
      benefit: "Buy 1 Mana Potion",
      icon: "fas fa-flask",
      baseCost: 80,
    },
    life: {
      cost: 800,
      benefit: "+1 Life (Max 5)",
  ...
  openShop: function () {
    ...
    UI.addToLog(
      "Welcome to the Mystical Shop! Spend your gold wisely.",
      "event",
    );
  },
  ...
  isUpgradeMaxedOut: function (upgradeType) {
    switch (upgradeType) {
      ...
      case "strength":
        return (
          gameState.character.strength >=
          gameState.character.getMaxStrengthCap()
        );
      case "agility":
        return (
          gameState.character.agility >= gameState.character.getMaxAgilityCap()
        );
      case "power":
        return (
          gameState.character.power >= gameState.character.getMaxPowerCap()
        );
      case "mastery":
        return (
          gameState.character.combatSpecialization.physicalMastery >=
            gameState.character.combatSpecialization.maxPhysicalMastery &&
          gameState.character.combatSpecialization.magicalMastery >=
            gameState.character.combatSpecialization.maxMagicalMastery
        );
      case "potion":
  ...
  buyUpgrade: function (upgradeType) {
    ...
    switch (upgradeType) {
      case "health":
        const healthIncrease = Math.min(
          15,
          gameState.character.maxMaxHp - gameState.character.maxHp,
        );
        if (healthIncrease > 0) {
          gameState.character.maxHp += healthIncrease;
          gameState.character.heal(healthIncrease);
          UI.addToLog(
            `Health increased! +${healthIncrease} Max HP (and healed to full)`,
            "victory",
          );
        } else {
      ...
      case "mana":
        const manaIncrease = Math.min(
          10,
          gameState.character.maxMaxMana - gameState.character.maxMana,
        );
        if (manaIncrease > 0) {
          gameState.character.maxMana += manaIncrease;
          gameState.character.restoreMana(manaIncrease);
          UI.addToLog(
            `Mana increased! +${manaIncrease} Max Mana (and restored to full)`,
            "victory",
          );
        } else {
      ...
      case "strength":
        ...
        if (strengthIncrease > 0) {
          ...
          UI.addToLog(
            `Strength training complete! +${strengthIncrease} Strength`,
            "victory",
          );
        } else {
      ...
      case "agility":
        const agilityIncrease = Math.min(
          2,
          gameState.character.getMaxAgilityCap() - gameState.character.agility,
        );
        if (agilityIncrease > 0) {
          gameState.character.agility += agilityIncrease;
          UI.addToLog(
            `Agility training complete! +${agilityIncrease} Agility (Defense)`,
            "victory",
          );
        } else {
      ...
      case "power":
        const powerIncrease = Math.min(
          2,
          gameState.character.getMaxPowerCap() - gameState.character.power,
        );
        if (powerIncrease > 0) {
          gameState.character.power += powerIncrease;
          UI.addToLog(
            `Power training complete! +${powerIncrease} Power (Magic Damage)`,
            "victory",
          );
        } else {
      ...
      case "mastery":
        const physicalIncrease = Math.min(
          1,
          gameState.character.combatSpecialization.maxPhysicalMastery -
            gameState.character.combatSpecialization.physicalMastery,
        );
        const magicalIncrease = Math.min(
          1,
          gameState.character.combatSpecialization.maxMagicalMastery -
            gameState.character.combatSpecialization.magicalMastery,
        );
        ...
        if (physicalIncrease > 0 || magicalIncrease > 0) {
          ...
          gameState.character.combatSpecialization.magicalMastery +=
            magicalIncrease;
          UI.addToLog(
            `Combat mastery enhanced! +${physicalIncrease} Physical, +${magicalIncrease} Magical Mastery`,
            "victory",
          );
        } else {
      ...
      case "life":
        if (gameState.lives < 5) {
          gameState.lives++;
          UI.addToLog(
            "Extra life granted! You feel more resilient.",
            "victory",
          );
        } else {
      ...
      case "potion":
        gameState.inventory.potions++;
        UI.addToLog(
          "Healing potion purchased! It's been added to your inventory.",
          "victory",
        );
        break;
      ...
      case "manaPotion":
        gameState.inventory.manaPotions++;
        UI.addToLog(
          "Mana potion purchased! It's been added to your inventory.",
          "victory",
        );
        break;
    }
    ...
    this.updateShopDisplay();
    UI.addToLog(
      `Purchase complete! Remaining gold: ${gameState.inventory.gold}`,
      "event",
    );
  },
};
...
const GameController = {
  ...
  continueGameStart: function () {
    ...
    UI.updateStory(
      "<i class='fas fa-door-open'></i> You enter the dungeon depths. The air grows cold as darkness surrounds you...",
    );
    UI.addToLog("=== GAME START ===", "start");
    UI.addToLog(
      `Welcome, ${gameState.selectedClass}! Your adventure begins...`,
      "start",
    );
    UI.showExplorationButtons();
    UI.updateDisplay();
  ...
  restAtCamp: function () {
    ...
    UI.updateStory(
      '<i class="fas fa-bed"></i> You rest at your camp, recovering strength and mana...',
    );
    UI.addToLog(
      `Resting... Recovered ${hpRestore} HP and ${manaRestore} mana.`,
      "rest",
    );
  },
  ...
  usePotion: function () {
    ...
    UI.updateStory(
      '<i class="fas fa-flask"></i> You drink a healing potion, feeling revitalized!',
    );
  ...
  useManaPotion: function () {
    ...
    UI.updateStory(
      "<i class='fas fa-flask'></i> You drink a mana potion, feeling magical energy surge through you!",
    );
  ...
  if (gameState.inCombat && gameState.currentEnemy) {
    ...
    if (spell.effectiveAgainst.includes(enemyName)) {
      effectivenessEl.innerHTML =
        '<i class="fas fa-star"></i> <span style="color: #43ea7c">Super effective vs ' +
        enemyName +
        "!</span>";
      effectivenessEl.classList.remove("hidden");
    } else if (spell.weakAgainst.includes(enemyName)) {
      effectivenessEl.innerHTML =
        '<i class="fas fa-exclamation-triangle"></i> <span style="color: #ff6b6b">Weak vs ' +
        enemyName +
        "...</span>";
      effectivenessEl.classList.remove("hidden");
    } else {
...
function updateSpellDropdown() {
  ...
  if (
    !gameState.character.knownSpells ||
    gameState.character.knownSpells.length === 0
  ) {
...
// ===============================================
// GAME INITIALIZATION (Robust init even if DOMContentLoaded already fired)
// ===============================================
(function initGame() {
  const start = () => {
    try {
      ...
      // Re-throw to be caught by error overlay in index.html
      throw e;
    }
  };
