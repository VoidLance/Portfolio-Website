...
const UI = {
    ...
    showSpellChoiceUI: function(spellChoices) {
        ...
        if (this._spellChoiceActive) return;
        ...
        spellChoices.forEach((spellName, index) => {
            ...
            btn.onclick = () => {
                ...
                // Regardless of replacement flow, hide all choice buttons now to enforce single selection per level
                disableAndRemoveAllChoices();
                if (!needsReplacement && !gameState.inCombat && !gameState.isGameOver) {
                    this.showExplorationButtons();
                }
                // If needsReplacement is true, showSpellReplaceUI will render its own UI next
            };
